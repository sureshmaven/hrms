...
namespace HRMSApplication.Controllers
{
    ...
    public class DutyController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            // Initialize StartDate and EndDate to current local time to avoid default year 0001 in the view
            var now = GetCurrentTime(DateTime.Now);
            var lmodel = new OD_OtherDuty
            {
                Loginmode = lCredentials.LoginMode,
                VistorFrom = selected,
                StartDate = now,
                EndDate = now
            };
        ...
        public JsonResult DeputationHistorySearchViews(string StartDate, string EndDate)
        {
            ...
            try
            {
                ...
                {
                    // Normalize to support both "dd/MM/yyyy" and "dd-MM-yyyy" coming from the UI
                    string normalizedStart = (StartDate ?? string.Empty).Replace('-', '/');
                    string normalizedEnd = (EndDate ?? string.Empty).Replace('-', '/');

                    string[] sa = normalizedStart.Split('/');
                    string[] sa1 = normalizedEnd.Split('/');

                    if (sa.Length != 3 || sa1.Length != 3)
                    {
                        // If format is unexpected, return no data instead of throwing
                        return Json(Enumerable.Empty<object>(), JsonRequestBehavior.AllowGet);
                    }

                    string strNew = sa[2] + "/" + sa[1] + "/" + sa[0];   // yyyy/MM/dd
                    string strNew1 = sa1[2] + "/" + sa1[1] + "/" + sa1[0]; // yyyy/MM/dd
        ...
        public JsonResult ODHistoryView(FormCollection form)
        {
            ...
            try
            {
                ...
                // Dates come from UI as dd/MM/yyyy – parse explicitly to avoid culture issues
                string startDateText = form["Stdate"];
                string endDateText = form["Endate"];
                ...
                string[] startParts = startDateText.Split('/');
                string[] endParts = endDateText.Split('/');

                string startReformatted = startParts[2] + "/" + startParts[1] + "/" + startParts[0]; // yyyy/MM/dd
                string endReformatted = endParts[2] + "/" + endParts[1] + "/" + endParts[0];       // yyyy/MM/dd

                DateTime FromDate = DateTime.ParseExact(startReformatted, "yyyy/MM/dd", null);
                DateTime ToDate = DateTime.ParseExact(endReformatted, "yyyy/MM/dd", null);
                ...
                if (Applieddate == "Request")
                {
                    var lResults = (from deput in ldeputation
                                    ...
                                    where (deput.StartDate.Date >= lStartdate.Date && deput.EndDate.Date <= lEnddate.Date)
                                          || (deput.EndDate.Date >= lStartdate.Date && deput.StartDate.Date <= lEnddate.Date)
                                    select new
                                    {
                                        deput.Id,
                                        emp.EmpId,
