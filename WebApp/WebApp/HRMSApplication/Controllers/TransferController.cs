...
namespace HRMSApplication.Controllers
{
    ...
    public class TransferController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var items3 = Facade.EntitiesFacade.GetAllBranches().Where(a => a.IFSCCode != "TSHEADOFF" && a.Name != "TGCAB-CTI").Select(x => new Branches
            {
                Id = x.Id,
                Name = x.Name
            }).Distinct();
        ...
        public ActionResult Create(EmpTransfer transfer, FormCollection from)
        {
            ...
            else
            {
                ...
                else if (transfer.RadioValue2 == "42" && transfer.Type == "Transfer" && transfer.Transfer_Type == "TemporaryTransfer")
                {
                    ...
                    db.SaveChanges();
                    DateTime lstartdate = GetCurrentTime(DateTime.Now.Date);
                    if (res.UpdatedDate >= transfer.EffectiveFromT && lstartdate <= transfer.EffectiveToT)
                    {
                        Employees lemployee = emplist.Where(a => a.EmpId == lid).FirstOrDefault();
                        ...
                        lemployee.CurrentDesignation = ldata.CurrentDesignation;
                        var requiredShiftType = "0"; // default = General

                        var designationName = db.Designations
    .Where(d => d.Id == lemployee.CurrentDesignation)
    .Select(d => d.Name)
    .FirstOrDefault();

                        if (!string.IsNullOrEmpty(designationName) && designationName.StartsWith("Attender"))
                        {
                            requiredShiftType = "1"; // Attender
                        }// Pick correct shift for new branch + designation
                        lemployee.Shift_Id = db.Shift_Master
                            .Where(s => s.BranchId == lemployee.Branch
                                     && s.ShiftType == requiredShiftType
                                     && DateTime.Now >= s.ValidFrom
                                     && DateTime.Now <= s.ValidTo)
                            .Select(s => s.Id)
                            .FirstOrDefault();
    ...
    .FirstOrDefault();
                ...
                else if (transfer.RadioValue2 == "43" && transfer.Type == "Transfer" && transfer.Transfer_Type == "TemporaryTransfer")
                {
                    ...
                    db.SaveChanges();
                    DateTime lstartdate = GetCurrentTime(DateTime.Now.Date);
                    if (lstartdate >= transfer.EffectiveFromT && lstartdate <= transfer.EffectiveToT)
                    {
                        Employees lemployee = emplist.Where(a => a.EmpId == lid).FirstOrDefault();
                        lemployee.Branch = transfer.NewBranchT;
                        lemployee.Department = lId;
                        lemployee.CurrentDesignation = ldata.CurrentDesignation;// Check designation to decide shift type
                        var requiredShiftType = "0"; // default = General
                        var designationName = db.Designations
    .Where(d => d.Id == lemployee.CurrentDesignation)
    .Select(d => d.Name)
    .FirstOrDefault();
                        ...
                        // Pick correct shift for new branch + designation
                        lemployee.Shift_Id = db.Shift_Master
                            .Where(s => s.BranchId == transfer.NewBranchT
                                     && s.ShiftType == requiredShiftType
                                     && DateTime.Now >= s.ValidFrom
                                     && DateTime.Now <= s.ValidTo)
                            .Select(s => s.Id)
                   ...
                   else if (transfer.EffectiveToT != null && lstartdate >= transfer.EffectiveToT)
                    {
                        Employees lemployee = emplist.Where(a => a.EmpId == lid).FirstOrDefault();
                        lemployee.Branch = ldata.Branch;
                        lemployee.Department = ldata.Department;
    ...
    .FirstOrDefault();
                   ...
                   else if (lstartdate >= transfer.EffectiveFromT)
                    {
                        Employees lemployee = emplist.Where(a => a.EmpId == lid).FirstOrDefault();
                        lemployee.Branch = transfer.NewBranchT;
                        lemployee.Department = lId;
    ...
    .FirstOrDefault();
                            ...
                            // Pick correct shift for new branch + designation
                            lemployee.Shift_Id = db.Shift_Master
                                .Where(s => s.BranchId == transfer.NewBranchPT
                                         && s.ShiftType == requiredShiftType
                                         && DateTime.Now >= s.ValidFrom
                                         && DateTime.Now <= s.ValidTo)
                                .Select(s => s.Id)
