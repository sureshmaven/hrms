    ...
    public class MemoController : Controller
    {
        ...
        public JsonResult GetEmployeeData(string empcode)
        {
            ...
            try
            {
                ...
                int lcontrollingAuthority = Convert.ToInt32(lControllingAuthority);
                //Session["lcontrols"] = lcontrol;
                Session["lSancation"] = lsancationcontrol;
                Session["lcontrolling"] = lcontrollingAuthority;
                Employees lcontrolling = Facade.EntitiesFacade.GetEmpTabledata.GetById(lcontrollingAuthority);
                ...
                return Json(new { lshortnmaeAjax = lshortname, ldesigAjax = desig, lbranchAjax = branchs, lSanctioningAuthorityAjax = lsancationing.ShortName, Status = lstatus1, lControllingAuthorityAjax = lcontrolling.ShortName }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                lresult = ex.Message;
        ...
        public ActionResult MemoCreate(Latememo latememo, string MemoType)
        {
            ...
            var reasonforleave = latememo.ReasonForLeave;
            ...
            if (latememo.leavetype == "Select")
            {
                latememo.leavetype = "";
            }
            latememo.leavetype = latememo.leavetype;
            latememo.Leaveapplieddate = latememo.Leaveapplieddate;
            //var controlling = db.Employes.Where(a => a.EmpId == latememo.Empid).Select(a => a.ControllingAuthority).FirstOrDefault();
            //latememo.Responsedate = null;
           
           // latememo.controllingauthority = controlling;
        ...
        [HttpGet]
        public JsonResult GetMemoDetails(string empCode, int month, int year)
        {
            try
            {
                System.Diagnostics.Debug.WriteLine($"GetMemoDetails called with: empCode={empCode}, month={month}, year={year}");

                var memoTable = Tbus.EmpMonthlyLateMemoDetails(empCode, year, month);

                string memoText = string.Empty;
                if (memoTable != null && memoTable.Rows.Count > 0)
                {
                    // Join all rows' first column values separated by new lines
                    var parts = new System.Collections.Generic.List<string>();
                    foreach (System.Data.DataRow row in memoTable.Rows)
                    {
                        parts.Add(row[0]?.ToString());
                    }
                    memoText = string.Join("\n", parts);
                }
                else
                {
                    memoText = "No Late Attendance Found.";
                }

                return Json(new { success = true, data = memoText }, JsonRequestBehavior.AllowGet);
            }
            catch (System.Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error in GetMemoDetails: {ex}");
                return Json(new { success = false, data = "Internal Server Error" }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        public JsonResult MemoListData()
        {
            ...
            var lResult = (from memolist in latememo
                           ...
                           select new
                           {
                               ...
                               memolist.Noofdays,
                               memolist.Priornoticegivendays,
                               memolist.ReasonForLeave,
                               memolist.leavetype,
                               memolist.Leaveapplieddate,
                               memolist.MemoType,
        ...
        public ActionResult Edit(int? id)
        {
            ...
            ViewBag.SA = sanction;
            var LeaveType = db.LeaveTypes;
            //var LeaveType = Facade.EntitiesFacade.GetAllBanks().Select(x => new Banks
            //var LeaveType = Facade.EntitiesFacade.LeavesTypesRepositoryFacade.Select(x => new Banks
            //{
            //    Id = x.Id,
            //    Name = x.Name
            //}).Distinct();

            // ViewBag.LeaveTypes = new SelectList(LeaveType, "Id", "Name");
            TempData["RolePages"] = LoginHelper.GetCurrentUserPages();
           
            //Session["RolePages"] = LoginHelper.GetCurrentUserPages();

            return View(lmemo);
        }

        [HttpPost]
        public JsonResult Edit(Latememo latememo, int Id, string memotype, int noofdays, string duedate, string memodetails, int Priornoticegivendays, string ReasonForLeave, string leavetype)
        {
            ...
            try
            {
                ...
                latememo1.Priornoticegivendays = Priornoticegivendays;
                latememo1.ReasonForLeave = ReasonForLeave;
                if (latememo1.leavetype == "Select")
                {
                    latememo1.leavetype = "";
                }
                latememo1.leavetype = latememo1.leavetype;
                db.Entry(latememo1).State = EntityState.Modified;
                db.SaveChanges();
        ...
        public ActionResult Latememo(string MemoType, int memoid)
        {
            ...
            ViewBag.Year = ViewBag.Date.ToString("yyyy");
            ...
            ViewBag.leavetype = memodata.leavetype;
            ViewBag.Leaveapplieddate = memodata.Leaveapplieddate;
            ViewBag.reasonforleave = memodata.ReasonForLeave;
            ViewBag.IsDate = memodata.IssueDate.ToString("dd.MM.yyyy");
            string controlling = db.Employes.Where(a => a.EmpId == EmpId).Select(a => a.ControllingAuthority).FirstOrDefault();
            string sanction = db.Employes.Where(a => a.Id.ToString() == controlling).Select(a => a.ShortName).FirstOrDefault();
            string controllingEmpid = db.Employes.Where(a => a.Id.ToString() == controlling).Select(a => a.EmpId).FirstOrDefault();
            ViewBag.SA = sanction;
            ViewBag.CEmpid = controllingEmpid;
            ...
            int CDesignation = (int)db.Employes.Where(a => a.EmpId == EmpId).Select(a => a.ControllingDesignation).FirstOrDefault();
            string cDesignation = db.Designations.Where(a => a.Id == CDesignation).Select(a => a.Code).FirstOrDefault();
            int cbranch = (int)db.Employes.Where(a => a.EmpId == EmpId).Select(a => a.ControllingBranch).FirstOrDefault();
            string cbranchs = db.Branches.Where(a => a.Id == cbranch).Select(a => a.Name).FirstOrDefault();
            int cdept = (int)db.Employes.Where(a => a.EmpId == EmpId).Select(a => a.ControllingDepartment).FirstOrDefault();
            string cdepts = db.Departments.Where(a => a.Id == cdept).Select(a => a.Name).FirstOrDefault();
            if (branch == 43)
            {
            ...
            if (cbranch == 43)
            {
                ViewBag.cbranch = cdepts;
            }
            else
            {
                ViewBag.cbranch = cbranchs;
            }
            ViewBag.Designation = Designation;
            ViewBag.CDesignation = cDesignation;
            ...
            string violation = "";
            string ssrno = "";
            if (ViewBag.leavetype == "Casual Leave")
            {
                provision = "Should apply at least 3 days in advance as per SSR No.27(vi)";
                violation = " 3 days advance notice is not given.";
                   // "<br/>" +
                   //"ii) Applied after 3 days of proceeding on leave.";
                ssrno = "27(vi)";
            }
            if (ViewBag.leavetype == "Privilage Leave")
            {
                provision = "Should apply at least 7 days in advance as per SSR No.29(v)";
                violation = " 7 days advance notice is not given. ";
                  //  "<br/>" +
                  //" Applied after 7 days of proceeding on leave.";

                ssrno = "29(v)";
            }
            ViewBag.Provision = provision;
            ...
            else if (MemoType == "Late Applied Leave") 
            {
                // return View("/Views/Memo/Controllinglateappliedleave.cshtml");
                return View("/Views/Memo/LateAppliedLeave.cshtml");
            }
            return View();
        }

        //public ActionResult EmployeeMemo()
        //{
        //    string Userid = lCredentials.EmpId;
        //    var CId = db.Employes.Where(a => a.EmpId == Userid).Select(a => a.Id).FirstOrDefault();
        //    //var CEmpid = db.Employes.Where(a=> a.ControllingAuthority == Id.ToString()).Select(a => a.EmpId).FirstOrDefault();
        //    var Empid = db.Latememo.Where(a => a.controllingauthority == CId.ToString()).Select(a => a.Empid).FirstOrDefault();
        //    if(Empid != null)
        //    {
        //        return View("/Views/Memo/Controllinglateappliedleave.cshtml");
        //    }
        ...
        public ActionResult EmployeeMemo()
        {
            ...
            var lResult = (from memolist in latememo
                           ...
                           && (memolist.Status == "Complete" || memolist.Status == "Pending")
        ...
        public ActionResult EmployeeMemoLeave(string MemoType, int memoid)
        {
            var empid = lCredentials.EmpId;
            ViewBag.MemoType = MemoType;
            ViewBag.memoid = memoid;
            ...
            if (ViewBag.leavetype == "Privilage Leave")
            {
                ...
                violation = " 7 days advance notice is not given.";
                  //  "\n" + "<br>" +
                  //"ii) Applied after 7 days of proceeding on leave.";
            ...
            if (MemoType == "Late Applied Leave")
            {
                // return View("/Views/Memo/Controllinglateappliedleave.cshtml");
                return View("/Views/Memo/Controllinglateappliedleave.cshtml");
            }
