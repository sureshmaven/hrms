CREATE PROCEDURE [dbo].[sp_dm_loan_pfloans_lt3] @emp_code integer,  @mn varchar(20)
AS  
begin
declare @fy NVARCHAR(100);
declare @fyQry NVARCHAR(100);
set @fyQry = 'select @fy = dbo.getFY('+@mn+');';
execute sp_executesql 
    @fyQry, 
    N'@fy NVARCHAR(100) OUTPUT', 
    @fy = @fy output;
--* check loan exists or not *
	if not exists (select * from pr_emp_adv_loans where emp_code = @emp_code and loan_type_mid = 20)
	begin
		...
		where m.EmpId = @emp_code and l.LoanId='PFLT3' and PMonth=@mn-1 order by m.EmpId
		...
		@total_installment,@remaining_installment, @principal_installment, @interest_installment, @completed_installment, @sanction_date, 
		 @installment_start_date, @installment_end_date, @installment_amount, @total_recovered_amount, @fm, @fy,@loanslno, @loan_repaid
...
select 
 ...
 @interest_amount_recovered_one=l.CumIntRepaid+l.IntRepaid,
 ...
 @os_principal_amount=l.Amount-l.CumLoanRepaid-l.LoanRepaid,
 @os_interest_amount=l.IntAmount-l.CumIntRepaid
 from   TEST2.dbo.PoldLoan l   join TEST2.dbo.PEmpMast m on m.eid=l.eid
where 
m.EmpId = @emp_code  and  l.LoanId = 'PFLT3' and PMonth=@mn-1
...
@total_principal_installments,@total_interest_installments, @os_principal_amount, @os_interest_amount,@loanslno 
	...
	--- Execute procedure for loans in loans adjustment table 
		...
		select 
		 ...
		 @interest_amount_recovered_adj=((l.intamount+l.intaccured)-(l.IntRepaid+l.CumIntRepaid)),
	    -- @interest_amount_paid_month = l.CumIntRepaid,
	     @interest_amount_paid_month = l.IntRepaid,
		 @os_interest_amount_adj=l.IntAmount-l.CumIntRepaid,
		 @principal_amount_recovered_one_adj=l.LoanRepaid,
		 ...
		 where l.LoanId='PFLT3' and  m.EmpId = @emp_code and l.loanslno = @loanslno order by l.Eid

		--if exists(select p.* from test2.dbo.PoldLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'PFLT3' and p.PMonth=@mn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
		--begin
...
----- Inserting in prevo datsa
	...
	begin
     ...
     where l.LoanId='PFLT3' and  m.EmpId = @emp_code  and loanslno = @loanslno  and pmonth = @pmn order by l.Eid
 ...
 --check in poldloan with loanslid and @pmn
	...
	-- @interest_amount_paid_month = l.CumIntRepaid,
	 ...
	 @principal_amount_recovered_one_adj=l.LoanRepaid,
	 @fest_recovery = l.recovery,
	 @remaing_amount_adj = l.Amount- l.CumLoanRepaid-l.InstAmt,
	 @fm = CAST((convert(date,convert(date,left(l.PMonth,4)+'-'+right(l.PMonth,2)+'-01'),102)) as varchar),
	  @completed_installment=CAST(l.InstNo as varchar),
	  @interest_open_amount=l.IntAmount-CumIntRepaid
	from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.PoldLoan l  on m.Eid=l.Eid 
	where  l.LoanId='PFLT3' and l.loanslno = @loanslno and l.pmonth = @pmn  order by l.Eid
	...
	@fm = CAST((convert(date,convert(date,left(l.PMonth,4)+'-'+right(l.PMonth,2)+'-01'),102)) as varchar)
	 from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.PoldLoan l  on m.Eid=l.Eid 
	where l.LoanId='PFLT3' and loanslno = @loanslno and pmonth = @pmn  order by l.Eid

	if exists(select p.* from test2.dbo.PoldLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'PFLT3' and p.PMonth=@pmn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
	begin
	print('inside '+cast(@pmn as varchar(20)));
	exec sp_insert_pfloans_adjustments @emp_code, @loan_code, @os_priciapl_open_amount, @interest_accured_adj, @interest_open_amount,
