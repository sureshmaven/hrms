...
case when (month(@fm)>=04 ) then Year(@fm)+1 else year(@fm) end as fy , @fm as fm, pslip.pf as own_share,case when pfo.PFprin  is null then 0 else pfo.PFprin end as own_share_open, 
...
Update new_num set last_num=@initValue where table_name='pr_ob_share';
--select * from pr_ob_share;
...
set @initValue=(select last_num+1 from new_num where table_name='pr_ob_share')
print(@yearstart);
--old slip @yearstart
...
select @initValue as id, (select id from Employees where EmpId=@empcode) as emp_id, @empcode as emp_code
,case when (month(@yearone)>=04 ) then Year(@yearone)+1 else year(@yearone) end as fy , @yearone as fm, pslip.pf as own_share,@own_share_open as own_share_open, (isnull(@own_share,0) + isnull(@own_share_open,0)) as own_share_total, 0 as  own_share_intrst_open,
...
select @initValue as id, (select id from Employees where EmpId=@empcode) as emp_id, @empcode as emp_code,case when (month(@yearone1)>=04 ) then Year(@yearone1)+1 else year(@yearone1) end as fy , @yearone1 as fm, pslip.pf as own_share,@own_share_open1 as own_share_open, (isnull(pslip.pf,0) + isnull(@own_share_open1,0)) as own_share_total, 0 as  own_share_intrst_open,
