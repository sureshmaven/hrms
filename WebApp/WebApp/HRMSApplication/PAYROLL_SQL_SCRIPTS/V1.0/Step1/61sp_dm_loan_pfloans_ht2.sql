CREATE PROCEDURE [dbo].[sp_dm_loan_pfloans_ht2] @emp_code integer,  @mn varchar(20)
AS  
begin
...
--* check loan exists or not *
	if not exists (select * from pr_emp_adv_loans where emp_code = @emp_code and loan_type_mid = 17)
	begin
		...
		where m.EmpId = @emp_code and l.LoanId='PFHT2' and PMonth=@mn-1 order by m.EmpId
...
select 
 ...
 from   TEST2.dbo.PoldLoan l   join TEST2.dbo.PEmpMast m on m.eid=l.eid
where 
m.EmpId = @emp_code  and  l.LoanId = 'PFHT2' and PMonth=@mn-1
...
@total_principal_installments,@total_interest_installments, @os_principal_amount, @os_interest_amount,@loanslno 
	...
	--- Execute procedure for loans in loans adjustment table 
		...
		select 
		 ...
		 @interest_amount_recovered_adj=l.IntAmount-l.CumIntRepaid,
	    -- @interest_amount_paid_month = l.CumIntRepaid,
		 ...
		 where l.LoanId='PFHT2' and  m.EmpId = @emp_code and l.loanslno = @loanslno order by l.Eid

		--if exists(select p.* from test2.dbo.PoldLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'PFHT2' and p.PMonth=@mn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
		--begin
...
where l.LoanId='PFHT2' and  m.EmpId = @emp_code  and loanslno = @loanslno  and pmonth = @pmn order by l.Eid
set @installment_amount=@adj_installment_amount;
 --check in poldloan with loanslid and @pmn
	...
	from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.PoldLoan l  on m.Eid=l.Eid 
	where  l.LoanId='PFHT2' and l.loanslno = @loanslno and l.pmonth = @pmn  order by l.Eid
	...
	@fm = CAST((convert(date,convert(date,left(l.PMonth,4)+'-'+right(l.PMonth,2)+'-01'),102)) as varchar)
	 from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.PoldLoan l  on m.Eid=l.Eid 
	where l.LoanId='PFHT2' and loanslno = @loanslno and pmonth = @pmn  order by l.Eid

	if exists(select p.* from test2.dbo.PoldLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'PFHT2' and p.PMonth=@pmn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
	begin
