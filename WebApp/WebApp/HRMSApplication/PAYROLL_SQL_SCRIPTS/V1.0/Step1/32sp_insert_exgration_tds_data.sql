...
(@StartValues+1,@fys,@fms,(select id from employees where empid = @employeecodes),@employeecodes,29,'per_earn',(select p.Earnings from [TEST2].dbo.EXT$ p left outer join [TEST2].dbo.pempmast m on p.EmpID=m.Eid where p.EmpID=@employeecodes),1,'General', 1000);
