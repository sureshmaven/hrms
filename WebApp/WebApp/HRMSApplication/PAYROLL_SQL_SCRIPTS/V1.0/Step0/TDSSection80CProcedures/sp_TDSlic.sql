...
CREATE table #licdetails (id int, name NVARCHAR(50),amount decimal(18,2))

Insert into #licdetails values ((select '3' as id),(select 'LIC' as name),(select sum(paydedu.dd_amount) as amount from pr_emp_payslip_deductions paydedu where paydedu.dd_name = 'LIC' and emp_code = @empcode and paydedu.payslip_mid in(select id from pr_emp_payslip where emp_code=@empcode and fy =@fy )) )
...
WHILE exists (SELECT top 1 * from #licdetails)
begin

set @id=(select last_num from new_num where table_name='pr_emp_tds_section_deductions')
set @pfid=(select top 1 id from #licdetails)
set @name=(select top 1 name from #licdetails)
set @amount=(select top 1 amount from #licdetails)
--select * from pr_emp_payslip where fm='2020-06-01' and spl_type='Regular'
set @projamount=(select (paydedu.dd_amount) as amount from pr_emp_payslip_deductions paydedu where paydedu.dd_name = 'LIC' and emp_code = @empcode and paydedu.payslip_mid in(select id from pr_emp_payslip where emp_code=@empcode and fm=@currentmonth and spl_type='Regular')and paydedu.dd_amount>0)
...
Delete from #licdetails where id=@pfid and name=@name and amount=cast(@amount as decimal(18,2));
set @pfid=0;
set @amount=0;
set @totamount=0;
end
Drop table #licdetails;
