CREATE procedure sp_TDShouseloan2dRetiring as
...
set @projamount=(select case when (paydedu.dd_amount) is null then 0 else (paydedu.dd_amount) end as amount from pr_emp_payslip_deductions paydedu where paydedu.dd_name = 'Housing Addl.Loan - 2D' and emp_code = @empcode and paydedu.payslip_mid in(select id from pr_emp_payslip where emp_code=@empcode and fm=@currentmonth and spl_type='Regular')and paydedu.dd_amount>0)
