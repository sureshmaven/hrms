...
Insert into dim_emp(dim_eid,Empid,FirstName,LastName,DoJ,RetirementDate,Designation_id,Department_id,Branch_id,Shift_Id) select Id,Empid,FirstName,LastName,DoJ,RetirementDate,CurrentDesignation,Department,Branch,Shift_Id from Employees where retirementdate>'2020-04-01'
...
SET @FIRSTDAY=(SELECT  DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0))
SET @LASTDAY=(SELECT  DATEADD(yy, DATEDIFF(yy, 0, GETDATE()) + 2, -1))
SET @ID=1;
