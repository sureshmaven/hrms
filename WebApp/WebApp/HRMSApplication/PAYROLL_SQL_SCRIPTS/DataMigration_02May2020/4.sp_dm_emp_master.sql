...
into #pr_emp_lic_details_migration from [TEST2].dbo.PEmpOthDet p join [TEST2].dbo.pempmast m on p.EID = m.EID where  p.PrmType='LIC' and m.Empid=@employeecode;
...
into #pr_emp_hfc_details_migration from [TEST2].dbo.PEmpOthDet p join [TEST2].dbo.pempmast m on p.EID = m.EID where  p.PrmType='HFC' and m.Empid=@employeecode;
