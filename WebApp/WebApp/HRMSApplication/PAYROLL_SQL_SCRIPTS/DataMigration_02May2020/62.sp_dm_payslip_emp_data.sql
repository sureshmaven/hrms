...
if(@a=1) 
begin 
insert into dm_emp_mn_task values (@empid,@year1,'PS,PROM,IIB,ENCASH,ADHOC,TDS,');
end
else if(@a=2)
insert into dm_emp_mn_task values (@empid,@year1,'PS,PROM,IIB,ENCASH,ADHOC,');
else if(@a=3)
insert into dm_emp_mn_task values (@empid,@year1,'PS,RD,PE,PD,JA,PR,CB,OTDS,PROM,IIB,ENCASH,ADHOC,');
set @id=@id+1
end
