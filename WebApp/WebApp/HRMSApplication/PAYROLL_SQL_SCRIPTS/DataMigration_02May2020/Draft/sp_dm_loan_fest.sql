...
--* check loan exists or not *
	...
	begin
		...
		where m.EmpId = @emp_code and l.LoanId='FEST' order by m.EmpId
		...
		select  
		 ...
		 @principal_amount_recovered = l.CumLoanRepaid,
		 @total_amount_recovered=l.Recovery,
		 ...
		 @os_principal_amount=l.Amount,
		 @loanslno = l.LoanSlNo,
		 @os_remaining_amount_paid = l.Amount-l.CumLoanRepaid
	...
	--- Execute procedure for loans in loans adjustment table 
		...
		select 
		 @os_priciapl_open_amount=Recovery,
		...
		if exists(select p.* from test2.dbo.PLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'FEST' and p.PMonth=@mn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
		begin
...
--exec sp_dm_loan_fest 247, 201908
