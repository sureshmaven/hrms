...
as 
begin
declare @syr NVARCHAR(100);
declare @eyr NVARCHAR(100);
set @syr=SUBSTRING(@pmonth,0,5)
set @eyr=@syr+1;
...
declare @pf int;
set @pf =(select p.pf from TEST2.dbo.[PEncash ] p join [TEST2].dbo.pempmast m on p.EID = m.EID where m.Empid = @employeecode and p.pmonth=@pmonth)
print @pf
print 'a'
if (@pf is null)
begin
...
@pf as dd_provident_fund,p.Tds as dd_income_tax,p.Ptax as dd_prof_tax,p.SUBCLUB as dd_club_subscription,
p.TGASSN as dd_telangana_officers_assn,p.Netpay as net_amount,p.Deductions as deductions_amount, 0 as active,1000 as trans_id,p.SBASICDA as spl_da,p.SBASICALW as spl_allw, 'Encashment' as spl_type ,1 as final_process,p.CEOALLW as err_ceoallw
into #pr_emp_payslip
...
from TEST2.dbo.[PEncash ] p join [TEST2].dbo.pempmast m on p.EID = m.EID  
where  
--p.paydate between '2019-04-01' and GETDATE() and p.pmonth>201903 and 
m.Empid = @employeecode and p.pmonth=@pmonth;
...
net_amount,deductions_amount,active,trans_id,spl_da,spl_allw,spl_type,final_process,err_ceoallw) select * from #pr_emp_payslip;
set @pf=0;
...
--where  p.paydate between '2019-04-01' and GETDATE() and e.pmonth>201903 and m.Empid = @employeecode and e.pmonth=@pmonth)
if exists(select * from TEST2.dbo.[PEncash ] p join [TEST2].dbo.pempmast m on p.EID = m.EID 
where  
--p.paydate between '2019-04-01' and GETDATE() and p.pmonth>201903 and 
m.Empid = @employeecode and p.pmonth=@pmonth)
begin
...
insert into PLE_Type values((select top 1  id from employees where empid=@employeecode),(select top 1  ControllingAuthority from employees where empid=@employeecode),
(select top 1  SanctioningAuthority from employees where empid=@employeecode),'PL ENCASHMENT','PL ENCASHMENT',123456,getdate(),'Approved',null,
(select top 1  TotalExperience from employees where EmpId=@employeecode),3,'Encashment',@syr,(select top 1  LeaveBalance from EmpLeaveBalance 
where empid=(select top 1  id from employees where empid=@employeecode) and LeaveTypeId=3 and year=@eyr),(select top 1 p.EncashDays from test2.dbo.PEncashment p join test2.dbo.PEMPMAST m on p.eid=m.eid where m.Empid=@employeecode and p.pmonth=@pmonth),(select Branch from employees where empid=@employeecode),
(select top 1  Department from employees where empid=@employeecode),(select top 1  CurrentDesignation from employees where empid=@employeecode),
(select top 1  LeaveBalance from EmpLeaveBalance where empid=(select top 1  id from employees where empid=@employeecode) and LeaveTypeId=3 and year=@eyr),
1,1,@fy,@fm,(select top 1  id from pr_emp_payslip where fm= @fm and fy = @fy and emp_code=@employeecode and spl_type='Encashment'));
end
...
(select top 1 id from pr_emp_payslip where fm= @fm and  emp_code = @employeecode and spl_type='Encashment'), @ID ,
...
(select top 1 id from pr_emp_payslip where fm= @fm and  emp_code = @employeecode and spl_type='Encashment'), @IDsss ,
