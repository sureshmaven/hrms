CREATE PROCEDURE [dbo].[sp_dm_sub_loans] @emp_code integer,  @fmn integer
AS  
...
insert into #two_sub select distinct(LoanId) as loan_id from [TEST2].dbo.PoldLoan where LoanId='HOUS1';
...
if exists (select * from test2.dbo.PoldLoanDet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and LoanId = @l_id and priority=4 and pmonth= @fmn-1)
begin
declare @date_disburse_four date; 
declare @loan_amount_four int;  
declare @interest_rate_four float;
declare @interest_accured_four float; 
declare @principal_amount_recovered_four int;
declare @interest_amount_recovered_four float; 
declare @total_amount_recovered_four float;
declare @principal_start_date_four date;
declare @principal_end_date_four date;
declare @interest_start_date_four date; 
declare @interest_end_date_four date;
declare @total_principal_installments_four int;
declare @total_interest_installments_four int; 
declare @os_principal_amount_four int; 
declare @os_interest_amount_four float;


select 
 @emp_code=CAST(m.Empid as varchar),
 @loan_code=d.LoanId,
 @date_disburse_four=cast(d.SanDate as Date),
 @loan_amount_four=d.Amount,
 @interest_rate_four=d.IntRate,
 @interest_accured_four=d.intamount+d.intaccured,
 @principal_amount_recovered_four=d.LoanRepaid+d.CumLoanRepaid,
 @interest_amount_recovered_four=d.IntRepaid+d.CumIntRepaid,
 @total_amount_recovered_four=d.LoanRepaid+d.CumLoanRepaid+d.IntRepaid+d.CumIntRepaid,
 @principal_start_date_four=cast(d.SanDate as Date),
 @principal_end_date_four=cast(d.SanDate as Date),
 @interest_start_date_four=cast(d.SanDate as Date),
 @interest_end_date_four=cast(d.SanDate as Date),
 @total_principal_installments_four=0,
 @total_interest_installments_four=0,
 @os_principal_amount_four=(d.Amount-(d.LoanRepaid+d.CumLoanRepaid)),
 @os_interest_amount_four=((d.intamount+d.intaccured)-(d.IntRepaid+d.CumIntRepaid)),
 @os_total_balace_amount = d.Amount-d.CumLoanRepaid+d.IntAmount-d.CumIntRepaid
from [TEST2].dbo.PoldLoandet d join [TEST2].dbo.PEMPMAST m on d.eid=m.eid  
where 
d.Priority=4 and  m.EmpId = @emp_code  and  d.LoanId = @l_id and d.pmonth= @fmn-1

exec sp_insert_three_sub_Loans_priority4 @emp_code, @loan_code, @date_disburse_four, @loan_amount_four, @interest_rate_four,
@interest_accured_four, @principal_amount_recovered_four, @interest_amount_recovered_four, @total_amount_recovered_four,
@principal_start_date_four ,@principal_end_date_four, @interest_start_date_four,@interest_end_date_four,
@total_principal_installments_four,@total_interest_installments_four, @os_principal_amount_four, @os_interest_amount_four,@os_total_balace_amount,@loanslno 
end 
...
set @installment_amount=@adj_installment_amount;
 ...
 --- Check priority 4 ----
 if exists (select * from test2.dbo.poldloandet where loanslno = @loanslno and priority = 4 and pmonth = @pmn)
 begin
	...
	begin	
		...
		@principal_paid_amount=LoanRepaid
		from [TEST2].dbo.PoldLoanDet
		where LoanId=@l_id and loanslno = @loanslno and pmonth = @pmn and Priority = 4 
