CREATE PROCEDURE [dbo].[sp_insert_two_wheller_single_child] @emp_code int, @loan_code varchar(20),
@date_disburse date, @loan_amount int , @interest_rate float,@interest_accured float, 
@principal_amount_recovered_one int,@interest_amount_recovered_one float, 
@total_amount_recovered_one float, @principal_start_date date ,@principal_end_date date ,
@interest_start_date date,@interest_end_date_one date ,
...
@os_principal_amount,@os_interest_amount  ,0 ,@os_principal_amount,
case when @os_principal_amount= 0 then 1 else 0 end,
case when @os_interest_amount= 0 then 1 else 0 end,
