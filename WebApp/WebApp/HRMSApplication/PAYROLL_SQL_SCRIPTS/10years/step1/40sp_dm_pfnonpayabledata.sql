CREATE procedure [dbo].[sp_dm_pfnonrepayable]   as
...
CAST((convert(date,convert(date,left(Pfmonth,4)+'-'+right(Pfmonth,2)+'-01'),102)) as varchar)as fm
...
non_refAdvances.eid=mas.Eid where loantype='nonrefund'
...
--select * from #pfnonrepayable
 ...
 --declare @fms nvarchar(20);
 declare @ownsahre decimal(18,2);
 --declare @fms nvarchar(20);
 ...
 set @ownsahre=(select top 1 Fownshare from #pfnonrepayable)
 --set @eligamount = (select top 1 LLeast from #pfnonrepayable)
 --set @fms=(select top 1 Pfmonth from #pfnonrepayable)
 ...
 0 as active,1 as authorisation ,1 as process,trans_id,sactioned as sanctioned_amount,SanctionDT as sanction_date,ProcessDT as process_date,0 as bankshare_interest ,0 as ownshare_interest ,0 as vpf_interest ,0 as is_interest_calculated into #pfnonrepayable1 from #pfnonrepayable where Empcode=@empids 
--and Pfmonth=@fms;
and PfAccountno=@pfaccno and BasicDA=@basic  and Fownshare=@ownsahre;
...
drop table #pfnonrepayable1;
 ...
 delete from #pfnonrepayable where Empcode=@empids and PfAccountno=@pfaccno and BasicDA=@basic  and Fownshare=@ownsahre;
 end
