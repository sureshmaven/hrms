...
@remaing_amount_adj_festival, 
case when @loan_code = 'FEST'  then 0 else @interest_accured_adj end , 
case when @loan_code = 'FEST'  then 0 else @interest_amount_recovered_adj end , 
case when @loan_code = 'FEST'  then 0 else @interest_amount_paid_month end,
case when @loan_code = 'FEST'  then 0  else @principal_amount_recovered_one_adj end, 
@completed_installment,  @installment_amount, @paid_date, @paid_date,
...
--Insert into pr_emp_payslip_deductions(id,emp_id, emp_code, payslip_mid, dd_mid, 
--dd_name, dd_amount, dd_type, active, trans_id) 
--values
--(@idnew0,@emp_id, @emp_id, 2050, @loanidnew, 'loanname', @installment_amount, 'Installment', 1,  @transidnew);
----select * from pr_emp_adv_loans_child