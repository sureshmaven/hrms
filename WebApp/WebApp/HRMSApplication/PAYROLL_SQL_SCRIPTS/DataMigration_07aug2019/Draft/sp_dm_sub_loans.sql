...
insert into #two_sub select distinct(LoanId) as loan_id from [TEST2].dbo.PLoan where LoanId='VEH4W';
...
@total_installment=((CAST(l.Period as int))+(CAST(ceiling(cast(IntPeriod as int)/4*(3)) as varchar))),
...
@sanction_date=cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) 
...
if exists (select * from test2.dbo.PLoanDet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and LoanId = @l_id and SlNo=1)
  begin
...
if exists (select * from test2.dbo.PLoanDet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and LoanId = @l_id and SlNo=2)
begin
...
select 
 ...
 cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) else 
 convert(date,convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as Date)) as varchar),
 @total_principal_installments_two=CAST(ceiling(cast(Period as int)/4*(3)) as varchar),
 @total_interest_installments_two=CAST(ceiling(cast(IntPeriod as int)/4*(3)) as varchar),
...
@total_principal_installments_two,@total_interest_installments_two, @os_principal_amount_two, @os_interest_amount_two,@loanslno 

end 
end

if not exists (select distinct d.loanslno from [TEST2].dbo.ploan p join [TEST2].dbo.ploandet d on d.loanslno=p.LoanSlNo  and d.LoanId =@l_id
group by d.loanslno Having count(p.loanslno)=2 )

begin
    --Execute procedure fest loans in child Table
	    exec sp_insert_fest_loans_child @emp_code, @loan_code, @date_disburse, @total_amount, @principal_recoverd, @total_recovered_amount,
        @principal_start_date, @principal_end_date, @total_principal_installments, @os_principal_amount,@loanslno
...
exec sp_insert_loans_adjustments @emp_code, @loan_code, @os_priciapl_open_amount, @interest_accured_adj,  @interest_open_amount,
 ...
 @remaing_amount_adj, @fest_recovery, @total_recovered_amount_festival, @remaing_amount_adj_festival, @fm, @fy,@loanslno,@priority
end
...
--check for prev. month adj.
 ...
 begin
	...
	begin	
		 ...
		 @interest_open_amount = IntAmount,
...
--exec sp_dm_sub_loans 374,201909
