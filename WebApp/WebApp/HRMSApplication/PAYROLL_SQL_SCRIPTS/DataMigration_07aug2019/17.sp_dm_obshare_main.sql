...
@fy as fy, @fm as fm, pslip.pf as own_share,case when pfo.PFprin  is null then 0 else pfo.PFprin end as own_share_open, (isnull(pslip.pf,0) + isnull(pfo.PFprin,0))  as own_share_total, case when pfo.PFint  is null then 0 else pfo.PFint end  as  own_share_intrst_open,
CASE 
WHEN ((pslip.Grpay * 8.50 )/100)>1250 
THEN (pslip.pf-1250)ELSE (pslip.pf-((pslip.Grpay * 8.50 )/100)) END AS 
bank_share, case when pfo.Bankprin  is null then 0 else pfo.Bankprin end  as bank_share_open,CASE WHEN ((pslip.Grpay * 8.50 )/100)>1250 THEN (pslip.pf-1250) ELSE (pslip.pf-((pslip.Grpay * 8.50 )/100)) end  + pfo.Bankprin as bank_share_total,
case when pfo.Bankint  is null then 0 else pfo.Bankint end as bank_share_intrst_open, 
pslip.VPF as vpf,case when pfo.VPFprin  is null then 0 else pfo.VPFprin end  as vpf_open, (isnull(pslip.VPF,0) + isnull(pfo.VPFprin,0)) as vpf_total,case when pfo.VPFint  is null then 0 else pfo.VPFint end  as  vpf_intrst_open,CASE 
WHEN ((pslip.Grpay * 8.33 )/100)>1250 
THEN 1250 ELSE (pslip.Grpay * 8.33 )/100 END AS 
...
WHEN ((pslip.Grpay * 8.50 )/100)>1250 
THEN (pslip.pf-1250)ELSE (pslip.pf-((pslip.Grpay * 8.50 )/100)) END AS 
bank_share,@bank_share_open as bank_share_open,isnull(@bank_share,0) + isnull(@bank_share_open,0) as bank_share_total,
0 as bank_share_intrst_open, 
pslip.VPF as vpf,@vpf_open as vpf_open, (isnull(@vpf,0) + isnull(@vpf_open,0)) as vpf_total, 0 as  vpf_intrst_open,CASE 
WHEN ((pslip.Grpay * 8.33 )/100)>1250 
THEN 1250 ELSE (pslip.Grpay * 8.33 )/100 END AS 
...
@own_share_inst_total as own_share_intrst_total,CASE 
WHEN ((pslip.Grpay * 8.50 )/100)>1250 
THEN (pslip.pf-1250)ELSE (pslip.pf-((pslip.Grpay * 8.50 )/100)) END AS 
bank_share,@bank_share_open1 as bank_share_open,CASE 
WHEN ((pslip.Grpay * 8.50 )/100)>1250 
...
pslip.VPF as vpf,@vpf_open1 as vpf_open, (isnull(pslip.VPF,0) + isnull(@vpf_open1,0)) as vpf_total, 0 as  vpf_intrst_open,@vpf_inst_total as vpf_intrst_total,CASE 
WHEN ((pslip.Grpay * 8.33 )/100)>1250 
THEN 1250 ELSE (pslip.Grpay * 8.33 )/100 END AS 
