...
Phc as phy_handicapped,Houseprovid as house_provided,
...
insert into #codes_master select id, name,emp_master_code,type from pr_earn_field_master where emp_master_code!=''
...
insert into #codes_master_all select id, name,emp_master_code,type from pr_allowance_field_master where emp_master_code!='' and type='EMPA'
...
insert into #codes_master_allded select id, name,emp_master_code,type from pr_deduction_field_master where emp_master_code!='' and  type = 'EPD'
...
into #pr_emp_lic_details_migration from [TEST2].dbo.PEmpOthDet p join [TEST2].dbo.pempmast m on p.EID = m.EID where m.Dor>GETDATE() and p.PrmType='LIC' and m.Empid=@employeecode;
...
into #pr_emp_hfc_details_migration from [TEST2].dbo.PEmpOthDet p join [TEST2].dbo.pempmast m on p.EID = m.EID where m.Dor>GETDATE() and p.PrmType='HFC' and m.Empid=@employeecode;
