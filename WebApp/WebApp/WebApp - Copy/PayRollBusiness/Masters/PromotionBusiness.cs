...
{
    public class PromotionBusiness : BusinessBase
    {
        public PromotionBusiness(LoginCredential loginCredential) : base(loginCredential)
        {
        ...
        public async Task<string> GetEmpPRCategories()
        {
            string query1 = "SELECT id,category FROM pr_emp_categories";
            DataTable dt1 = await _sha.Get_Table_FromQry(query1);
            var resultJson = JsonConvert.SerializeObject(dt1);
            return resultJson;
        }
        public async Task<string> GetEmpStopsalDetails(int EmpId)
        {
            if (EmpId != 0)
            {
                string Querysuspended = " select * from pr_month_attendance where status in('Suspended') and active = 1 and emp_code in (" + EmpId + ")";
                DataTable dtsuspended = await _sha.Get_Table_FromQry(Querysuspended);
                if (dtsuspended.Rows.Count > 0)
                {

                    return "E#Error Promotion#Suspended Status Employee is not eligible for Promotion";
                }
                string Querysuspended1 = " select * from pr_month_attendance where status in('StopSalary') and active = 1 and emp_code in (" + EmpId + ")";
                DataTable dtsuspended1 = await _sha.Get_Table_FromQry(Querysuspended1);
                if (dtsuspended1.Rows.Count > 0)
                {

                    return "E#Error Promotion#StopSalary Status Employee is not eligible for Promotion";
                }
            }
            return null;
        }
        public async Task<string> GetEmpDesignations()
        {
            string query1 = "SELECT Id,Name FROM Designations";
            DataTable dt1 = await _sha.Get_Table_FromQry(query1);
            var resultJson = JsonConvert.SerializeObject(dt1);
        ...
        public async Task<IList<PrPromotion>> GetEmpPRDetails(int Empid)
        {
            ...
            string qryGetPRfields = "SELECT top(1)  eprom.Id,eprom2.EmpId, FORMAT (eprom.EffectiveFrom, 'yyyy-MM-dd') as promotion_date,eprom.NewDesignation,eprom.fy,eprom.fm,eprom.category,eprom.new_basic,FORMAT (eprom.incre_due_date, 'yyyy-MM-dd') as incre_due_date,eprom.senoirity_order,eprom.active,case when eprom.Id is null then 'N' else 'U' end as row_type FROM Employees eprom2 left join Employee_Transfer eprom on eprom2.EmpId = eprom.EmpId and eprom.active = 1 and eprom.authorisation=0 join Designations d on eprom2.CurrentDesignation= d.id WHERE eprom2.EmpId='" + Empid + "' order by trans_id desc";
        ...
        public async Task<IList<PrPromotion>> GetEmpPRAuthDetails(int Empid)
        {
            string qryGetPRfields = "SELECT top(1) eprom.Id,eprom2.EmpId, FORMAT (eprom.EffectiveFrom, 'yyyy-MM-dd') as promotion_date,d.name as desig,eprom.fy,eprom.fm,eprom.category,eprom.new_basic as basic_pay_fixed,FORMAT(eprom.incre_due_date, 'yyyy-MM-dd') as incre_due_date,eprom.senoirity_order,eprom.active,case when eprom.Id is null then 'N' else 'U' end as row_type FROM Employees eprom2 join Employee_Transfer eprom on eprom2.Id = eprom.EmpId  and Authorisation = 1 join Designations d on eprom2.CurrentDesignation = d.id WHERE eprom2.EmpId='" + Empid + "' order by id desc";
            ...
            if (dt.Rows.Count > 0)
            {
                string pdate =Convert.ToDateTime(dt.Rows[0]["promotion_date"]).ToString("dd-MM-yyyy");
                string idate =Convert.ToDateTime(dt.Rows[0]["incre_due_date"]).ToString("dd-MM-yyyy");
                lstPRData.Add(new PrPromotion
                {
                    // id = Convert.ToInt32(dt.Rows[0]["id"]),
                    display = "New Category :",
        ...
        public async Task<string> AddPRPayDetails(CommonPostDTO Values)
        {
            ...
            try
            {
                ...
                if (Values.object1 != null)
                {
                    ...
                    string qrey = "Select EmpId,fy,fm,category,NewDesignation,new_basic,EffectiveFrom,incre_due_date,senoirity_order,authorisation,active from Employee_Transfer where Active=1 and authorisation=0 and EmpId=" + empidpr + "; ";
                    ...
                    if (dt1.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dt1.Rows)
                        {
                            ...
                            string desig = dt1.Rows[0]["desig"].ToString();
                           // string basic_pay = dt1.Rows[0]["basic_pay"].ToString();
                            string basic_pay_fixed = dt1.Rows[0]["basic_pay_fixed"].ToString();
                            string promotion_date = Convert.ToDateTime(dt1.Rows[0]["promotion_date"]).ToString("yyyy/MM/dd");
                            ...
                            string qrey2 = "Update Employee_Transfer set active=1,EmpId='" + empidpr + "',fy='" + FY + "',fm='" + FM + "',category='"+cate+ "',NewDesignation='" + des + "',new_basic=" + bpayfixed + ",EffectiveFrom='" + prdate + "',incre_due_date='" + incremdate + "',senoirity_order='" + seniority + "',authorisation=0  where empid=" + EfEmpid + " and active=1; ";
                            sbqry.Append(qrey2);
                        }
                    }
                    else
                    {
                        string qry = "INSERT INTO Employee_Transfer (id,empid,fy,fm,category,NewDesignation,new_basic,EffectiveFrom,incre_due_date,senoirity_order,active,trans_id,authorisation) VALUES(@idnew" + NewNumIndex + ",(select id from employees where empid=" + Values.EntityId + "), " + FY + ",'" + FM + "','" + cate + "','" + des +  "','" + bpayfixed + "',' " + prdate + "','" + incremdate + "','" + seniority + "', 1, @transidnew,0); ";
                        sbqry.Append(qry);

                    }
                    string qry2 = " select id from designations where code='" + des + "'";
                ...
                //    sbqry.Append(qry3);
                    ...
                    //4. transaction touch
                    sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.I, "Employee_Transfer", "@idnew" + NewNumIndex, lempid.ToString()));
                ...
                {
                    return "I#Promotion#Promotion Details Data Added Successfully..!!";
                }
                else
                {
