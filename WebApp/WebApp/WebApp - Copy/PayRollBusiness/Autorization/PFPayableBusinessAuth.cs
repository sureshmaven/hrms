...
{
    public class PFPayableBusinessAuth : BusinessBase
    {
        public PFPayableBusinessAuth(LoginCredential loginCredential) : base(loginCredential)
        {
        }
        ...
        public async Task<string> getPayableEmpDetails(string empCode)
        {
            ...
            try
            {
                ...
                string qry = "select pf.pf_account_no as pf_no,pf.purpose_of_advance as padvid,adv.purpose_name as purpose,pr_emp_adv_loans.total_amount,pf.pf_loans_id as loanid,pf.amount_applied_for as amountapp," +
                     " pf.[(basic+DA)*months_1] as basicData,pf.rate_of_interest as rate,pf.amount_applied_for_2 as amountapplied2,pf.calculating_months as calmon,[netownshare+25%netbankshare_3] as netshare,[least_of_3] as leastt,pf.[gross_salary] as grossamt,pf.net_salary as netamt,pf.net_minus_pf as netminuspf,pf.[1/3rd_of_gross_salary] as onethirdg,pf.total_outstanding_loan as total,pf.amount_recommended_for_sanction as sanction,pf.active from pr_emp_pf_repayable_loan pf left outer join pr_purpose_of_advance_master adv on pf.purpose_of_advance = adv.id left outer join pr_loan_master on pr_loan_master.loan_id=pf.pf_loans_id left outer  join pr_emp_adv_loans on pr_emp_adv_loans.loan_type_mid=pr_loan_master.id and pr_emp_adv_loans.emp_code=" + Convert.ToInt32(empCode) + " where pf.active=1 and pf.authorisation=0 and pf.emp_code=" + Convert.ToInt32(empCode);
                ...
                {
                    return "E#PF Payable# No Data Found to Authorise";
                    ...
                    //      "active = 1 ;";
                    //    string qry9 = "select amount_applied_for as pfloan2,emp_code from  pr_emp_pf_repayable_loan where active=1 and authorisation=0 and process=0 and pf_loans_id=1 and emp_code =" + Convert.ToInt32(empCode) + " ;";
                    //    string qry10 = "select amount_applied_for as pfloan2,emp_code from  pr_emp_pf_repayable_loan where active=1 and authorisation=0 and process=0 pf_loans_id=2 and emp_code =" + Convert.ToInt32(empCode) + " ;";
        ...
        public async Task<string> savePFPayableData(PFPayable Values)
        {
            try
            {
                ...
                qry = " update pr_emp_pf_repayable_loan set authorisation =1 where purpose_of_advance=" + Values.purposeType + " " +
                " and emp_code=" + Convert.ToInt32(emp_code) + " and active=1 and authorisation=0 and process=0;";
                sbqry.Append(qry);
                ...
                {
                    return "I#PF Payable#PF Payable Data Authorised Successfully..!!";
                }
                else
                {
