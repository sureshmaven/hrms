                ...
                if (fromdate != "")
                {
                    ...
                    while ((chunkEnd = start.AddDays(dayChunkSize)) <= end)
                    {
                        ...
                        if (empcode == "" && fromdate != string.Empty && todate != string.Empty && (branch == "All" || branch == "HeadOffice-All") && self == "False")
                        {
                            Empcheckintym = "select distinct case  when SUBSTRING(" + "D" + st1s + "," + "19, 2) Not In ('AB','CL','ML','PL','MTL','PTL','EOL','SCL','LOP','W-Off','C-OFF','CW-OFF','OD-OfWk')  then SUBSTRING(" + "D" + st1s + "," + "19, 2)" +
                                                ...
                                                " where leavesyear = " + s3 + " and startdate = '" + start + "' and (lt.code = 'CL' or lt.code = 'CW-OFF') and  l.Status in ('Approved','Pending','Forwarded')) and e.EmpId != 2 and e.RetirementDate >= convert(Date, '" + start + "')" + cond5 + " and sm.shifttype = (case when(select count(*)" +
                            ...
                            if (dt1.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow leavetym in dt1.Rows)
                                {
                                    ...
                                    empid.Add(empidcode);
                            //        string Weekly = "select e.empid as EmpId,wc.purpose as Purpose  from od_otherduty wc   " +
                            //               "join employees e on e.id = wc.empid where wc.purpose = 8 and wc.status ='Cancelled' and convert(date,wc.startdate) ='" + start3 + "'";

                            //        var dt2 = sh.Get_Table_FromQry(Weekly);

                            //        if (dt2.Rows.Count >= 0)
                            //        {
                            //            List<string> empidweeks = new List<string>();
                            //            foreach (DataRow empweekly in dt2.Rows)
                            //            {
                            //                weekpurpose = empweekly["Purpose"].ToString();
                            //                empidweek = empweekly["EmpId"].ToString();
                            //                empidweeks.Add(empidweek);  // Add each EmpId to the list
                            //                                            // cond8 = $"AND e.Empid NOT IN ({string.Join(",", empidweeks)})";
                            //                                            //string leaveweekly = "select lt.code from leaves l join leavetypes lt on l.leavetype = lt.id where empid = (select id from employees where empid = " + empidweek + ") AND l.startdate = '" + start3 + "'";
                            //                                            //var dt3 = sh.Get_Table_FromQry(leaveweekly);
                            //                                            // Existing leave check logic
                            //                string leaveweekly = $"SELECT lt.code FROM leaves l " +
                            //$"JOIN leavetypes lt ON l.leavetype = lt.id " +
                            //$"WHERE l.empid = (SELECT id FROM employees WHERE empid = {empidweek}) " +
                            //$"AND l.startdate = '{start3}'";
                            //                var dt3 = sh.Get_Table_FromQry(leaveweekly);

                            //                if (dt3.Rows.Count > 0)
                            //                {
                            //                    cond7 = " AND l.startdate = '" + start3 + "'";
                            //                }
                            //            }
                                        // Set cond8 after collecting all empidweeks
                                        //if (empidweeks.Count > 0)
                                        //{
                                        //    cond8 = $"AND e.Empid NOT IN ({string.Join(",", empidweeks)})";
                                        //}
                                        //
                                        if (empid.Count > 0)
                                        {
                                            cond9 = $"AND e.Empid NOT IN ({string.Join(",", empid)})";
                                        }
                                    }
                 ...
                 " when " + cond6 + " In ('AB','CL','ML','PL','MTL','PTL','EOL','SCL','LOP','W-Off','C-OFF','CW-OFF') or " + cond6 + " like '%OD%' then " + cond6 + " else SUBSTRING(" + "D" + st1s + "," + "7,5) + ( case when D" + st1s + "_meridian is not null then  subString(D" + st1s + "_meridian,4,2) else '' end )  end as EmpCheckOutTime," +
                            ...
                            if (dt1.Rows.Count == 0)
                            {
                                ...
                                //                }
                            //}
                                        // query = "select distinct t.User_Id as Empid from timesheet_logs t join employees e on e.empid = t.user_id and  io_time >= '" + start2 + " 00:00:00.000'  and io_time <= '" + start2 + " 23:59:59.000'  and  e.EmpId!=2 and e.RetirementDate >=convert(Date, '" + start + "') order by t.user_id";
