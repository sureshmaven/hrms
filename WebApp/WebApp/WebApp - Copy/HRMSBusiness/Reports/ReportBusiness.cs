...
namespace HRMSBusiness.Reports
{
    public class ReportBusiness
    {
        ...
        {
            SqlConnection con = new SqlConnection("Server=183.82.100.162; Database=Hrms_10052023_Before; user id=sa; password=Mavensoft;");
            ...
            using (con)
            {
                ...
                if (branchid == 0)
                {
                    using (SqlCommand cmd = new SqlCommand("select Empid, ShortName, 'Dccb_Employee' as name,case when((select distinct count(*) from timesheet_logs l" +
                        ...
                        "FORMAT(CAST('" + Date + "' AS date), 'yyyy/MM/dd') AS Date,'' as Shift from dccb_employees de where empid not in " +
                ...
                else
                {
                    using (SqlCommand cmd = new SqlCommand("SELECT emp.Empid,emp.ShortName,case when br.Name='OtherBranch' then dept.Name else br.Name end as name," +
                        ...
                        "emp.retirementdate>getdate() and emp. empid not in(select user_id  from [dbo].timesheet_logs as tb2 where cast(io_time as date)='" + Date + "') " +
        ...
        public DataTable allWDNew(string wd, string fromdt, string todt, string employeecode, string status)
        {
            ...
            string qry1 = " Select convert(varchar,w.EmpId) as EmpId , emp.Shortname, d.Code , case when b.Name='OtherBranch' then dept.Name when b.Name='HeadOffice' then dept.Name else b.Name end as branch," +
                ...
                " Where w.Status != 'Draft' and w.Status in ('Pending','Approved') and  emp.RetirementDate >= convert(Date, CAST(CAST(GETDATE() AS DATE) AS DATETIME)) and CurrentDesignation in (8, 7, 6, 5) and  (w.WDDate >= '" + fdate + "' and w.WDDate <='" + tdate + "') "
...
+ " join designations d on d.id = e.currentdesignation where e.RetirementDate >= convert(Date, CAST(CAST(GETDATE() AS DATE) AS DATETIME)) ";
        ...
        public DataTable headOffice(int DptId)
        {
            ...
            string qry = " select CAST(ROW_NUMBER() over(order by department) AS INT) Id, " +
                        ...
                        " on e.department = dp.id and e.RetirementDate >= convert(Date, CAST(CAST(GETDATE() AS DATE) AS DATETIME))  " + cond1;
            return sh.Get_Table_FromQry(qry);
        }
        ...
        public DataTable AllStaffMasterList()
        {
            ...
            string qry = " select CAST(ROW_NUMBER() over(order by e.Empid) AS INT) Id," +
                       ...
                       " e.Category,e.AadharCardNo,e.Graduation,e.PostGraduation,e.ProfessionalQualifications," +
                        " case when b.id = 43 then dp.name else b.name end as PresentWorkPlace,PersonalEmailId,OfficalEmailId " +
...
" where e1.RetirementDate >= convert(Date, CAST(CAST(GETDATE() AS DATE) AS DATETIME))  " + cond + "  order by e1.id   ";
        ...
        public DataTable AllLeavesHistoryList(string fromDate, string toDate, string datetype, string LeaveType, string LeaveStatus, string branch, string empcode, string datatype1, string dept)
        {
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != string.Empty && dept != string.Empty && empcode == string.Empty && branch != "0" && dept != "0" && branch == "null")
            {
                DateTime fdt = fromDate == "" ? DateTime.Now : Convert.ToDateTime(fromDate);
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "')  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))";
                //cond2 = "and e.Branch!=43";
                //dcon = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "')  and (status IN('Pending', 'Debit', 'Credit', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL'))   and deptid  in(" + dept + ")";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != string.Empty && dept != string.Empty && empcode == string.Empty && branch != "0" && dept != "0" && branch != "null" && dept != "null")
            {
                DateTime fdt = fromDate == "" ? DateTime.Now : Convert.ToDateTime(fromDate);
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "')  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and( branchid  in(" + branch + ") or deptid  in(" + dept + "))";
                //cond2 = "and e.Branch!=43";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch == "0" && empcode != string.Empty && dept == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and branchid not in(" + branch + ")";
                cond2 = "and e.Branch!=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch != "0" && dept != "null" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and (branchid  in(" + branch + ") or deptid in(" + dept + "))";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch != "0" && dept == "null" && empcode == string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and  branchid  in(" + branch + ")";
                cond2 = "and e.Branch!=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch == "All" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))and EmpId=" + empcode + " and BrDept! ='" + branch + "'";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != "0" && branch == "null" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))and EmpId=" + empcode + "";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch == "0" && dept == "null" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))and EmpId=" + empcode + " and branchid  not in(" + branch + ")";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && dept == "0" && empcode != string.Empty && branch == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and deptid not in(" + dept + ")";
                cond2 = "and e.Branch=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && dept != "0" && branch == "null" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and deptid  in(" + dept + ")";
                cond2 = "and e.Branch=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && dept != "0" && branch == "null" && empcode == string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and  deptid  in(" + dept + ")";
                cond2 = "and e.Branch=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && dept != "0" && empcode != string.Empty && branch == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))and EmpId=" + empcode + " and branchid   in(" + branch + ")";
                cond2 = "and e.Branch=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveType != "" && LeaveStatus == "ALL" && dept != "0" && dept == "null" && empcode != string.Empty && branch != "0" && branch == "null" && LeaveStatus != string.Empty)
            {
                ...
                {
                    cond1 = "where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                           " and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and EmpId=" + empcode + "";
                    cond2 = "and e.Branch=43";
                }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && dept != "0" && dept != "null" && empcode == string.Empty && branch == "null" && LeaveStatus != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = "where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       " and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and EmpId=" + empcode + " and deptid   in(" + dept + ")";
                cond2 = "and e.Branch=43";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && empcode != string.Empty && branch != string.Empty && branch == "0" && dept == "0")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and (status IN('Pending', 'Debit', 'Credit', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and EmpId=" + empcode + " ";
                //cond2 = "and e.Branch=43";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && empcode == string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && (dept == "0" || dept == "null"))
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled')) ";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus != "ALL" && empcode != string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && (dept == "0" || dept == "null"))
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and status = '" + LeaveStatus + "'  and EmpId in (" + empcode + " )";
                //cond2 = "and e.Branch=43";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && empcode != string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && (dept == "0" || dept == "null"))
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and EmpId in (" + empcode + " )";
                //cond2 = "and e.Branch=43";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus != "ALL" && empcode == string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && (dept == "0" || dept == "null"))
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and status = '" + LeaveStatus + "' ";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && LeaveStatus != "ALL" && empcode == string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && (dept == "0" || dept == "null"))
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')  ";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && empcode == string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && dept != "0" && dept != "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and status = '" + LeaveStatus + "' and (branchid not in (" + branch + ",43) or deptid in (" + dept + "))";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && empcode != string.Empty && branch != string.Empty && (branch == "0" || branch == "null") && dept != "0" && dept != "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where (Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103)" +
                    " <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF') and status = '" + LeaveStatus + "' and empId in (" + empcode + ") and (branchid not in (" + branch + ",43) or deptid in (" + dept + "))";
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch == "0")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and BrDept !='" + branch + "'";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch == "All")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))and  BrDept! ='" + branch + "'";
            }
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != "0" && branch != "null" && (dept == "0" || dept == "null") && empcode == string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and branchid ='" + branch + "'";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != "0" && branch != "null" && (dept == "0" || dept == "null") && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and branchid ='" + branch + "' and empid=" + empcode + "";
            }
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus != "ALL" && branch != "0" && branch != "null" && dept != "null" && dept == "0")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "')) " +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and (branchid in(" + branch + ") or deptid not in(" + dept + ",46))";
            }
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && LeaveStatus == string.Empty && branch != "0" && branch != "null" && dept != "null" && dept == "0")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "')) " +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and (branchid in(" + branch + ") or deptid not in(" + dept + ",46))";
            }
            ...
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType != "ALLTypes" && LeaveStatus == "ALL" && branch == "All")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " where ((Convert(date,AppliedDate,103) >='" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "') and  (Convert(date,AppliedDate,103) <= '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "'))" +
                       " and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and  LeaveType ='" + LeaveType + "'  and BrDept !='" + branch + "'";
            }
            else if (datetype == "Applied" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType != "ALLTypes" && LeaveStatus != "ALL" && branch == "All")
            {
                DateTime fdt = fromDate == "" ? DateTime.Now : Convert.ToDateTime(fromDate);
            ...
            {
                cond1 = " Where (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))";
            }
            ...
            {
                cond1 = " Where (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled')) and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && toDate != "-2" && fromDate != string.Empty && toDate != string.Empty && LeaveType != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && branch != string.Empty && empcode != string.Empty && branch != "0" && dept == "null")
            {
                DateTime fdt = fromDate == "" ? DateTime.Now : Convert.ToDateTime(fromDate);
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))and EmpId=" + empcode + " and  status = '" + LeaveStatus + "' and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and branchid   in (" + branch + ")";
                cond2 = "and e.Branch!=43";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch == "0" && empcode != string.Empty && dept == "null")
            {
                ...
                cond1 = " Where ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and branchid  not in (" + branch + ",43) ";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch != "0" && empcode != string.Empty && dept == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and branchid in(" + branch + ")";
            }
            //sigle leave types
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch == "0" && empcode == string.Empty && dept == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL')) ";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && branch == "0" && empcode != string.Empty && dept == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and EmpId=" + empcode + " and branchid not  in(" + branch + ")";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && dept == "0" && empcode != string.Empty && branch == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + " and deptid not in (" + dept + ")";
                //cond2 = "and e.Branch!=43";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && dept == "0" && empcode == string.Empty && branch == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and deptid not in (" + dept + ")";
                //cond2 = "and e.Branch=43";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && dept == "0" && empcode != string.Empty && branch == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and EmpId=" + empcode + " and deptid not  in(" + dept + ")";
                // cond2 = " and  lv.Status = '" + LeaveStatus + "'";
                //cond2 = "and e.Branch=43";
                //cond3 = " and lv.Type = '" + LeaveStatus + "'";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && empcode == string.Empty && (branch == "0" || branch == "null") && (dept == "0" || dept == "null"))
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) ";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && empcode != string.Empty && branch == "null" && dept == "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and EmpId=" + empcode + "  and  status = '" + LeaveStatus + "'  ";
                // cond2 = " and lt.Code='" + LeaveType + "'" + " and lv.Status = '" + LeaveStatus + "'";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && empcode == string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch != "0" && dept != "0" && branch != "null" && dept != "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and (branchid ='" + branch + "' or deptid ='" + dept + "')";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && empcode != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == string.Empty && branch != "0" && dept != "0" && branch != "null" && dept != "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and EmpId in (" + empcode + ") and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))  and (branchid ='" + branch + "' or deptid ='" + dept + "')";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch == "null" && branch != "0" && dept != "0" && dept != "null")
            {
                ...
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                   "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and  deptid in(" + dept + ")";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != "null" && branch != "0" && dept != "0" && dept == "null")
            {
                ...
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                   "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and  branchid in(" + branch + ")";
            }
            //end
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != "0" && branch != "null" && dept != "0" && dept != "null" && empcode == string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                   "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and  (branchid in(" + branch + ") or deptid in(" + dept + "))";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && branch != "0" && branch != "null" && dept != "0" && dept != "null" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                   "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and  (branchid in(" + branch + ") or deptid in(" + dept + ")) and empId in(" + empcode + ")";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && branch != "0" && branch == "null" && dept != "0" && dept != "null")
            {
                ...
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                     " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and  deptid in(" + dept + ")";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && branch != "0" && branch != "null" && dept != "0" && dept == "null")
            {
                ...
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and  branchid in(" + branch + ")";
            }
            //end
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && branch != "0" && branch != "null" && dept != "0" && dept != "null")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and  (branchid in(" + branch + ") or deptid in(" + dept + "))";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus == "ALL" && empcode != string.Empty && branch != "0" && dept != "0")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL')) and EmpId=" + empcode + " and  branchid in(" + branch + ")";

                cond2 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       "  and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled'))  and(LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL')) and EmpId=" + empcode + " and  deptid in(" + dept + ")";
            }
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && toDate != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != "ALL" && empcode != string.Empty && branch != "0" && dept != "0")
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and EmpId=" + empcode + " and  branchid in(" + branch + ")";

                cond2 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                     " and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "' and EmpId=" + empcode + " and  deptid in(" + dept + ")";
                //cond2 = " and  lv.Status = '" + LeaveStatus + "'";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && LeaveType != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus == "ALL" && branch != "0" && dept == "0" && empcode == string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled')) and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))   and (branchid  in (" + branch + ") or deptid not in(" + dept + "))";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && LeaveType != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus == "ALL" && branch != "0" && dept == "0" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled')) and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + "  and ( branchid  in (" + branch + ") or deptid  not in (" + dept + ") )";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && LeaveType != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus == "ALL" && branch == "0" && dept != "0" && empcode == string.Empty)
            {
                ...
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled')) and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF'))   and (branchid not  in (" + branch + ",43) or deptid   in (" + dept + "))";
            ...
            else if (datetype == "Request" && fromDate != "-1" && fromDate != string.Empty && toDate != "-2" && LeaveType != string.Empty && LeaveType == "ALLTypes" && LeaveStatus != string.Empty && LeaveStatus == "ALL" && branch == "0" && dept != "0" && empcode != string.Empty)
            {
                ...
                DateTime tdt = toDate == "" ? DateTime.Now : Convert.ToDateTime(toDate);
                cond1 = " Where  ((Convert(date,StartDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "'and '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or (Convert(date,EndDate,103) between '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' and'" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "') or '" + fdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103) or '" + tdt.ToString("yyyy-MM-dd 00:00:00.000") + "' between Convert(date,StartDate,103) and  Convert(date,EndDate,103))" +
                       " and (status IN('Pending', 'Debit','DebitCancelled', 'Credit','CreditCancelled', 'Forwarded', 'Approved', 'Denied', 'PartialCancelled', 'Cancelled')) and (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and EmpId=" + empcode + "  and (branchid not  in (" + branch + ",43) or deptid in(" + dept + "))";
            ...
            {
                cond1 = " Where (LeaveType IN('CL', 'PL', 'LOP','W-Off', 'All', 'C-OFF', 'SCL', 'EOL', 'MTL', 'ML', 'PTL','CW-OFF')) and status = '" + LeaveStatus + "'";
            }
