    ...
    public class LoginBus
    {
        ...
        public LoginResult getLoginInformation(string empid, string password)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                DateTime dbRetirementDate = (DateTime)dt.Rows[0]["RetirementDate"];
                //DateTime dbRelievingDate = (DateTime)dt.Rows[0]["RelievingDate"];
                // if (dbRetirementDate <= lcurrentDate || dbRetirementDate >= lcurrentDate)
                if (dbRetirementDate >= lcurrentDate) 
                {
                    lresult.Success = true;
                    lresult.EmpPkId = (int)dt.Rows[0]["EmpPk"];
            ...
            else
            {
        ...
        else if (roleid == 3 && (desig == 1 || desig == 2 || desig == 3 || desig == 4))
                {
                    ...
                    if (approve > 0)
                    {
                        ...
                        if (lcontrolling != null && lsanction == null)
                        {
                            retStr = "MemoSuper,MemoMain,DBMain,Payslip,MPMain,TsheetMain,LEMain,ODMain,LTMain,PLMain,POMain,WDAPLMain,LVMLV,ODOAL,LTCAL,PLPAL,EMPREP,COVID";
                        }
                        else if (lsanction != null && lcontrolling == null)
                        {
                            retStr = "MemoSuper,MemoMain,DBMain,Payslip,MPMain,LEMain,TsheetMain,ODMain,LTMain,PLMain,POMain,WDSHYMain,LVMLV,ODOAL,LTCAL,PLPAL,EMPREP,COVID";
                        }
                        else if (lsanction != null && lcontrolling != null)
                        {
                    ...
                    {
                        retStr = "MemoSuper,MemoMain,DBMain,Payslip,MPMain,TsheetMain,LEMain,ODMain,LTMain,PLMain,POMain,LVMLV,ODOAL,LTCAL,PLPAL,COVID";
                    }
