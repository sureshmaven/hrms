...
{
    [Authorize]
    public class PayslipPdfController : Controller
    {
        PayslipPdf pdfData;
        ...
        PayslipPdfBusiness pdf = new PayslipPdfBusiness(LoginHelper.GetCurrentUserForPR());
        ...
        //Create Payslip PDF
        public async Task<FileResult> CreatePdf(string EmpCode, string pdftypes, string btntype)
        {
            string emp_code = "";
            string dfm = "";
            string[] arrEmpId = EmpCode.Split(',');
            ...
            {
                DataTable GetEmpcode = _sha.Get_Table_FromQry("select emp_code,format(fm,'MMMM') as fm from  pr_emp_payslip where id=" + EmpCode + ";");
            ...
            foreach (string empId in arrEmpId)
            {
                ...
                decimal tot_deduction_amt = 0;
                foreach(var deduction in pdfData.Deductions)
                {
                    tot_deduction_amt += Convert.ToDecimal(deduction.Amount);
                }
                pdfData.deductionsamount =Convert.ToString(tot_deduction_amt);
                //Add Content to PDF   
            ...
            else
            {
                ...
                Response.AppendHeader("content-disposition", "inline; filename=" + strPDFFileName);
                return new FileStreamResult(workStream, "application/pdf");
            }
            //return File(workStream, "application/pdf", strPDFFileName);
        }
        public async Task<FileResult> Payslip(string EmpCode, string pdftypes, string btntype)
        {
            string emp_code = "";
            string dfm = "";
            string[] arrEmpId = EmpCode.Split(',');
