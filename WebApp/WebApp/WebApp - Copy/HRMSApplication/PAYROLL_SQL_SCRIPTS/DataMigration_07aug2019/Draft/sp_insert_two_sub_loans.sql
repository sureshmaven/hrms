CREATE PROCEDURE [dbo].[sp_insert_two_sub_Loans] @emp_code int ,@loan_code varchar(20),  @total_amount int , @total_installment int  ,
@remaining_installment int , @principal_installment int ,@interest_installment int ,
@completed_installment int, 
@sanction_date date , @installment_start_date date ,@installment_end_date date ,@method nvarchar(50),@interest_rate float, @installment_amount float ,
@interest_installment_amount float, @total_recovered_amount float,@vender_name varchar(30), @fm date,@fy int, @loanslno int
AS
print(@loan_code);
...
if not exists (select * from pr_emp_adv_loans where loan_sl_no = @loanslno)
begin
...
@installment_end_date, 'Interest to be Recovered at End' ,@interest_rate,@installment_amount,@interest_installment_amount,@total_recovered_amount,
'AB-HFL(HYD)',1,@transidnew,@fm,@fy, @loanslno);
