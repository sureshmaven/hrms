create procedure sp_dm_per_ded(@employeecode int,@pmonth varchar(20))
as 
begin
...
m on p.Eid=m.Eid  where FinYear=@fyPeriod and EdFlag='D' and m.Empid=@employeecode;
...
begin
	...
	if exists(select id from pr_deduction_field_master where type='per_ded' and name=@name) 
    begin
	--insertion
