...
--* check loan exists or not *
	...
	begin
		...
		where m.EmpId = @emp_code and l.LoanId='PFLT2' order by m.EmpId
...
select 
 ...
 from   TEST2.dbo.PLoan l   join TEST2.dbo.PEmpMast m on m.eid=l.eid
where 
m.EmpId = @emp_code  and  l.LoanId = 'PFLT2'
...
@total_principal_installments,@total_interest_installments, @os_principal_amount, @os_interest_amount,@loanslno 
	...
	--- Execute procedure for loans in loans adjustment table 
		...
		if exists(select p.* from test2.dbo.PLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'PFLT2' and p.PMonth=@mn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
		begin
