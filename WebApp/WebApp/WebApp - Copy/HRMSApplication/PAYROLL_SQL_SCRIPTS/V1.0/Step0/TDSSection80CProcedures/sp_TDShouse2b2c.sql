CREATE procedure sp_TDShouse2b2c as
...
CREATE table #houseloan2b2c (id int, name NVARCHAR(50),amount decimal(18,2))

Insert into #houseloan2b2c values ((select '7' as id),(select 'Housing Loan 2B-2C' as name),(select sum(paydedu.dd_amount) as amount from pr_emp_payslip_deductions paydedu where paydedu.dd_name = 'Housing Loan 2B-2C' and emp_code = @empcode and paydedu.payslip_mid in(select id from pr_emp_payslip where emp_code=@empcode and fy =@fy )) )
...
WHILE exists (SELECT top 1 * from #houseloan2b2c)
begin

set @id=(select last_num from new_num where table_name='pr_emp_tds_section_deductions')
set @pfid=(select top 1 id from #houseloan2b2c)
set @name=(select top 1 name from #houseloan2b2c)
set @amount=(select top 1 amount from #houseloan2b2c)
--select * from pr_emp_payslip where fm='2020-06-01' and spl_type='Regular'
set @projamount=(select (paydedu.dd_amount) as amount from pr_emp_payslip_deductions paydedu where paydedu.dd_name = 'Housing Loan 2B-2C' and emp_code = @empcode and paydedu.payslip_mid in(select id from pr_emp_payslip where emp_code=@empcode and fm=@currentmonth and spl_type='Regular')and paydedu.dd_amount>0)
...
print 'Housing Loan 2B-2C';
print @empcode;
print @amount;
print @totamount;
--select * from pr_emp_tds_section_deductions
Delete from #houseloan2b2c where id=@pfid and name=@name and amount=@amount;
set @pfid=0;
set @amount=0;
set @totamount=0;
end
Drop table #houseloan2b2c;
