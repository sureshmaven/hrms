CREATE procedure sp_TDSVPFRetiring as
...
set @fy= 2021;
select Empid,Retirementdate into #RetireEmployees from Employees where Retirementdate between @finstartyear and @finendyear 
and Empid in(select distinct emp_code from pr_emp_payslip where fm>=@finstartyear and fm<=@currentmonth) order by Empid
...
set @projamount=(select case when (paydedu.dd_amount) is null then 0 else (paydedu.dd_amount) end as amount from pr_emp_payslip_deductions paydedu where paydedu.dd_name = 'VPF Deduction' and emp_code = @empcode and paydedu.payslip_mid in(select id from pr_emp_payslip where emp_code=@empcode and fm=@currentmonth and spl_type='Regular')and paydedu.dd_amount>0)
