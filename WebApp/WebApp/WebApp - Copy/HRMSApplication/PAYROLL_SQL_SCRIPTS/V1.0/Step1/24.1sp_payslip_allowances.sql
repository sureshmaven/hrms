...
CREATE procedure sp_payslip_allowances (@fm nvarchar(20))as
begin
declare @id bigint;
declare @empid int;
...
SELECT m.Empid, m.FCLTYALLW,p.id  into #FacultyAllow FROM [test2].[dbo].[pempmast] m  inner join [pr_emp_payslip] p on  m.[Empid] collate SQL_Latin1_General_CP1_CI_AS = p.[emp_code] where  m.FCLTYALLW!=0 and p.fm>='2019-04-01' and p.fm<=@fm
