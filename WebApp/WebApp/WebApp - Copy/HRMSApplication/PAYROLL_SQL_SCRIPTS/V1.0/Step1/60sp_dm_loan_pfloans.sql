CREATE PROCEDURE [dbo].[sp_dm_loan_pfloans] @emp_code integer,  @mn varchar(20)
AS  
begin
...
--* check loan exists or not *
	...
	begin
		...
		declare @loan_repaid int;
		 ...
		 @total_recovered_amount=l.LoanRepaid+l.CumLoanRepaid+l.IntRepaid+l.CumIntRepaid,
		...
		where m.EmpId = @emp_code and l.LoanId='PFHT1' and PMonth=@mn-1 order by m.EmpId
...
select 
 ...
 from   TEST2.dbo.PoldLoan l   join TEST2.dbo.PEmpMast m on m.eid=l.eid
where 
m.EmpId = @emp_code  and  l.LoanId = 'PFHT1'  and PMonth=@mn-1
...
@total_principal_installments,@total_interest_installments, @os_principal_amount, @os_interest_amount,@loanslno 
	...
	--- Execute procedure for loans in loans adjustment table 
	    ...
	    -- @interest_amount_paid_month = l.CumIntRepaid,
		 ...
		 where l.LoanId='PFHT1' and  m.EmpId = @emp_code and l.loanslno = @loanslno and PMonth=@mn-1 order by l.Eid

		--if exists(select p.* from test2.dbo.PoldLoan p join test2.dbo.PEMPMAST m on p.eid=m.Eid where p.LoanId = 'PFHT1' and p.PMonth=@mn and m.Empid=@emp_code and p.LoanSlNo=@loanslno)
		--begin
...
where l.LoanId='PFHT1' and  m.EmpId = @emp_code  and loanslno = @loanslno  and pmonth = @pmn order by l.Eid
set @installment_amount=@adj_installment_amount;
 --check in poldloan with loanslid and @pmn
	...
	from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.PoldLoan l  on m.Eid=l.Eid 
	where  l.LoanId='PFHT1' and l.loanslno = @loanslno and pmonth = @pmn   order by l.Eid
