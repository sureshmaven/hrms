...
from [TEST2].dbo.PoldLoandet d join [TEST2].dbo.PEMPMAST m on d.eid=m.eid join [TEST2].dbo.PLoan l on d.loanslno=l.LoanSlNo 
where 
...
if  exists (select * from test2.dbo.PoldLoanDet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and loanslno = @loanslno and priority=0 and pmonth= @fmn-1)
  begin
select 
 @emp_code=CAST(m.Empid as varchar),
 @loan_code=LoanId,
