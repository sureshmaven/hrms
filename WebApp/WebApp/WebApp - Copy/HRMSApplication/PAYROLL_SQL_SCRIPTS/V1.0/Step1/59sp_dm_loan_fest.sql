CREATE PROCEDURE [dbo].[sp_dm_loan_fest] @emp_code integer,  @mn varchar(20)
AS  
begin
...
--* check loan exists or not *
	...
	--- Execute procedure for loans in loans adjustment table 
		...
		select 
		 @os_priciapl_open_amount= l.Amount-l.CumLoanRepaid,
		...
		@remaing_amount_adj_festival = l.Amount-(l.LoanRepaid+l.CumLoanRepaid)
		 from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.Poldloan l  on m.Eid=l.Eid 
...
----- Inserting in prevo datsa
	...
	begin
     ...
     where l.LoanId='FEST' and  m.EmpId = @emp_code  and loanslno = @loanslno  and pmonth = @pmn order by l.Eid
