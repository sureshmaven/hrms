CREATE PROCEDURE [dbo].[sp_dm_sub_loans_hladd] @emp_code integer,  @fmn varchar(20)
AS  
declare @fy NVARCHAR(100);
declare @fyQry NVARCHAR(100);
set @fyQry = 'select @fy = dbo.getFY('+@fmn+');';
...
insert into #two_sub select distinct(LoanId) as loan_id from [TEST2].dbo.PoldLoan where LoanId='HLADD';
...
--select * from Employees;
  if exists (select * from test2.dbo.PoldLoan p join test2.dbo.PEMPMAST m on p.Eid=m.eid where p.LoanId=@l_id and m.Empid=@emp_code and p.pmonth= @fmn-1)
  begin
...
--- Priority 0----
if exists (select * from test2.dbo.PoldLoanDet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and LoanId = @l_id and priority=0 and pmonth= @fmn-1)
begin
declare @date_disburse_zero date; 
declare @loan_amount_zero int;  
declare @interest_rate_zero float;
declare @interest_accured_zero float; 
declare @principal_amount_recovered_zero int;
declare @interest_amount_recovered_zero float; 
declare @total_amount_recovered_zero float;
declare @principal_start_date_zero date;
declare @principal_end_date_zero date;
declare @interest_start_date_zero date; 
declare @interest_end_date_zero date;
declare @total_principal_installments_zero int;
declare @total_interest_installments_zero int; 
declare @os_principal_amount_zero int; 
declare @os_interest_amount_zero float;


select 
 @emp_code=CAST(m.Empid as varchar),
 @loan_code=d.LoanId,
 @date_disburse_zero=cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) 
 else convert(date,convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) 
 end as varchar),
 @loan_amount_zero=d.Amount,
 @interest_rate_zero=d.IntRate,
 @interest_accured_zero=d.intamount+d.intaccured,
 @principal_amount_recovered_zero=d.LoanRepaid+d.CumLoanRepaid,
 @interest_amount_recovered_zero=d.IntRepaid+d.CumIntRepaid,
 @total_amount_recovered_zero=d.LoanRepaid+d.CumLoanRepaid+d.IntRepaid+d.CumIntRepaid,
 @principal_start_date_zero=cast(CASE WHEN LoanStart = '' or LoanStart is null then 
 cast(l.SanDate as Date) else convert(date,convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as varchar),
 @principal_end_date_zero=CAST(Dateadd(month,ceiling(cast(period as int)/4*(3)), cast(CASE WHEN LoanStart = '' 
 ...
 @interest_start_date_zero=CAST(Dateadd(month,ceiling(cast(period as int)+1), cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) else convert(date,
 convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as Date)) as varchar),
 @interest_end_date_zero=CAST(Dateadd(month,ceiling(cast(period as int)+1+ceiling(cast(IntPeriod as int)/4*(3))), 
 ...
 @total_principal_installments_zero=Period,
 @total_interest_installments_zero=IntPeriod,
 @os_principal_amount_zero=(d.Amount-(d.LoanRepaid+d.CumLoanRepaid)),
 @os_interest_amount_zero=((d.intamount+d.intaccured)-(d.IntRepaid+d.CumIntRepaid)),
...
where 
d.Priority=0 and  m.EmpId = @emp_code  and  l.LoanId = @l_id and d.pmonth= @fmn-1

exec sp_insert_three_sub_Loans_priority0 @emp_code, @loan_code, @date_disburse_zero, @loan_amount_zero, @interest_rate_zero,
@interest_accured_zero, @principal_amount_recovered_zero, @interest_amount_recovered_zero, @total_amount_recovered_zero,
@principal_start_date_zero ,@principal_end_date_zero, @interest_start_date_zero,@interest_end_date_zero,
@total_principal_installments_zero,@total_interest_installments_zero, @os_principal_amount_zero, @os_interest_amount_zero,@os_total_balace_amount,@loanslno 
end 
...
from [TEST2].dbo.PoldLoandet d join [TEST2].dbo.PEMPMAST m on d.eid=m.eid join [TEST2].dbo.PoldLoan l on d.loanslno=l.LoanSlNo 
where 
d.Priority=1 and  m.EmpId = @emp_code  and  l.LoanId = @l_id and d.pmonth= @fmn-1
...
from [TEST2].dbo.PoldLoandet d join [TEST2].dbo.PEMPMAST m on d.eid=m.eid join [TEST2].dbo.PoldLoan l on d.loanslno=l.LoanSlNo 
where 
d.Priority=2 and  m.EmpId = @emp_code  and  l.LoanId = @l_id and d.pmonth= @fmn-1
...
select 
 @emp_code=CAST(m.Empid as varchar),
 @loan_code=d.LoanId,
 @date_disburse_three=cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) 
 else convert(date,convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) 
 end as varchar),
 ...
 @principal_start_date_three=cast(CASE WHEN LoanStart = '' or LoanStart is null then 
 cast(l.SanDate as Date) else convert(date,convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as varchar),
 @principal_end_date_three=CAST(Dateadd(month,ceiling(cast(period as int)/4*(3)), cast(CASE WHEN LoanStart = '' 
 or LoanStart is null then cast(l.SanDate as Date) else convert(date,
 convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as Date)) as varchar),
 @interest_start_date_three=CAST(Dateadd(month,ceiling(cast(period as int)+1), cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) else convert(date,
 convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as Date)) as varchar),
 @interest_end_date_three=CAST(Dateadd(month,ceiling(cast(period as int)+1+ceiling(cast(IntPeriod as int)/4*(3))), 
 cast(CASE WHEN LoanStart = '' or LoanStart is null then cast(l.SanDate as Date) else 
 convert(date,convert(date,left(LoanStart,4)+'-'+right(LoanStart,2)+'-01'),102) end as Date)) as varchar),
 @total_principal_installments_three=Period,
 @total_interest_installments_three=IntPeriod,
...
from [TEST2].dbo.PoldLoandet d join [TEST2].dbo.PEMPMAST m on d.eid=m.eid join [TEST2].dbo.PoldLoan l on d.loanslno=l.LoanSlNo 
where 
d.Priority=3 and  m.EmpId = @emp_code  and  l.LoanId = @l_id and d.pmonth= @fmn-1
...
if not exists (select * from test2.dbo.PoldLoanDet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and loanslno = @loanslno and pmonth= @fmn-1)
  begin
select 
 @emp_code=CAST(m.Empid as varchar),
 @loan_code=LoanId,
...
declare @adj_installment_amount float;
select 
...
--check for prev. month adj.
 ...
 begin 
  select 
  @adj_installment_amount=CAST((case when  (l.InstAmt='0.00' and l.loanrepaid >'0.00' )then l.loanrepaid  else l.InstAmt end) as varchar) from   TEST2.dbo.PEmpMast m  join  TEST2.dbo.POldLoan l  on m.Eid=l.Eid 
  where l.LoanId=@l_id and  m.EmpId = @emp_code  and loanslno = @loanslno  and pmonth = @pmn order by l.Eid
  set @installment_amount=@adj_installment_amount;
  --- Check priority 0 ----
 if exists (select * from test2.dbo.poldloandet where loanslno = @loanslno and priority = 0 and pmonth = @pmn)
 begin
	...
	begin	
		 ...
		 where LoanId=@l_id and loanslno = @loanslno and pmonth = @pmn and Priority = 0
