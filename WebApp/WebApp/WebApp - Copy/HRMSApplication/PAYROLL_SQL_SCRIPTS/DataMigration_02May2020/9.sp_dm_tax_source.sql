...
set @pmonth1 = cast(left(@pset,4) as varchar(20))+'04' ;
...
union all select sum(Tds) as tax,'2' as id  from test2.dbo.POldslip p join test2.dbo.PEMPMAST m on p.Eid=m.Eid where m.Empid=@employeecode 
and Pmonth between @pmonth1 and @pmonth
union all
...
where  p.paydate between cast(left(@pset,4) as varchar(20))+'-04-01' and GETDATE() and e.pmonth>=@pmonth1 and m.Empid =371
union all 
select sum(Tds) as tax,'4' as id from TEST2.dbo.PAdhslip p  join [TEST2].dbo.pempmast m on p.EID = m.EID 
where p.paydate between cast(left(@pset,4) as varchar(20))+'-04-01' and GETDATE() and m.empid=@employeecode;
