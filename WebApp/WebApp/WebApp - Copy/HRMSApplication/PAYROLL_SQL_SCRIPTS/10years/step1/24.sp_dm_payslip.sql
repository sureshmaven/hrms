...
p.TGASSN as dd_telangana_officers_assn,p.Netpay as net_amount,p.Deductions as deductions_amount, 0 as active,1000 as trans_id,p.SBASICDA as spl_da,p.SBASICALW as spl_allw, 'Regular' as spl_type, 1 as final_process,p.CEOALLW as err_ceoallw 
into #pr_emp_payslip
...
--get codes
insert into #codes_master select id, name,code,type from pr_earn_field_master where code!='' and type not in('adhoc','EPF') union all 
...
insert into #codes_master_allded select id, name,code,type from pr_deduction_field_master where code!='' and type not in('adhoc','EPF') union all 
select id,loan_description as name,loan_id as code,'Loan' as type from pr_loan_master where --active=1 and 
loan_id!='PFL1' and loan_id!='PFL2' union all
select id,name,code,type from dm_payslip_loans where code not in ('PFHT1',
'PFHT2',
'PFLT2',
'PFLT3',
'PFLT4'
...
--declare @lastnumsss int; -- lastlic also having the same id, hence reuse it
...
designation,branch,working_days,lop,er_basic,er_da,er_cca,er_hra,er_interim_relief,er_telangana_inc,
gross_amount,dd_provident_fund,dd_income_tax,dd_prof_tax,dd_club_subscription,dd_telangana_officers_assn,
net_amount,deductions_amount,active,trans_id,spl_da,spl_allw,spl_type,final_process,err_ceoallw) select * from #pr_emp_payslipold;
...
--get codes
insert into #codes_master1 select id, name,code,type from pr_earn_field_master where code!=''and type not in('adhoc','EPF') union all 
select id, name,code,type from pr_allowance_field_master where code!='' ;
...
(select top 1 id from pr_emp_payslip where fm= @fm and  emp_code = @employeecode and spl_type='Regular' order by id desc), @ID1 ,
...
insert into #codes_master_allded1 select id, name,code,type from pr_deduction_field_master where code!='' and type not in('adhoc','EPF') union all 
select id,loan_description as name,loan_id as code,'Loan' as type from pr_loan_master where --active=1 and 
loan_id!='PFL1' and loan_id!='PFL2' union all
select id,name,code,type from dm_payslip_loans where code not in ('PFHT1',
'PFHT2',
