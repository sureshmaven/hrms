...
CREATE PROCEDURE [dbo].[sp_dm_sub_loans_four_wheeler] @emp_code integer,  @fmn integer
AS  
...
insert into #two_sub select distinct(LoanId) as loan_id from [TEST2].dbo.PoldLoan where LoanId='VEH4W';
...
select 
 ...
 @interest_rate_one=cast(CASE WHEN l.IntRate = '0.00' or l.IntRate is null then cast((select IntRate from TEST2.dbo.PEmpMast m join TEST2.dbo.PoldLoandet l on m.Eid=l.Eid where l.LoanId=@l_id and m.EmpId = @emp_code and l.pmonth= @fmn-1 ) as varchar)else CAST(l.IntRate as varchar) end as varchar),
...
--if exists (select * from test2.dbo.PoldLoandet where eid = (select eid from test2.dbo.PEMPMAST where Empid=@emp_code) and LoanId = @l_id and  loanslno = @loanslno
--and priority = 1 )
--begin
...
--  @os_priciapl_open_amount=d.Amount-d.CumLoanRepaid,
-- @remaing_amount_adj_festival = d.Amount-d.CumLoanRepaid - d.LoanRepaid,
-- @interest_accured_adj=d.IntAccured,
-- @interest_open_amount = d.IntAmount-d.CumIntRepaid,
...
-- @os_interest_amount_adj=d.IntAmount+d.IntAccured-d.CumIntRepaid-d.IntRepaid,
