...
(@new_id,@fy,@fm,(select id from employees where empid = @employeecode),@employeecode,(select id from pr_earn_field_master where type='per_earn' and name=@name),'per_earn',@amount,0,1000);
...
insert into pr_earn_field_master values(@StartValue+1,@name,'per_earn',0,1000,null,null,null);
...
(select id from pr_earn_field_master where name=@name and type='per_earn'),'per_earn',@amount,@sec,0,1000);
