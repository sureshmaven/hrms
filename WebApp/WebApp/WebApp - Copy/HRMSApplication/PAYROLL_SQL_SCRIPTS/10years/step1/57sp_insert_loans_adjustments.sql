CREATE PROCEDURE [dbo].[sp_insert_loans_adjustments] 
        @emp_code nvarchar(20) , 
		@loan_code varchar(20),
        @os_priciapl_open_amount integer,
		@principal_paid_amount integer,
		@interest_accured_adj int,
		@interest_open_amount int,
...
set @adv_loans_child_id = (select id from pr_emp_adv_loans_child where loan_sl_no = @loanslno and priority = 1 and emp_adv_loans_mid=@loan_id)

declare @adv_loans_child_id_two int;
set @adv_loans_child_id_two = (select id from pr_emp_adv_loans_child where loan_sl_no = @loanslno and priority = 2 and emp_adv_loans_mid=@loan_id)

declare @adv_loans_child_id_three int;
set @adv_loans_child_id_three = (select id from pr_emp_adv_loans_child where loan_sl_no = @loanslno and priority = 3 and emp_adv_loans_mid=@loan_id)


declare @adv_loans_child_id_four int;
set @adv_loans_child_id_four = (select id from pr_emp_adv_loans_child where loan_sl_no = @loanslno and priority = 4 and emp_adv_loans_mid=@loan_id)
...
--if exists(select id from pr_emp_adv_loans_child where loan_sl_no = @loanslno and priority = 1)
--begin
...
values
(@idnew0,@loan_id, case when  @priority = 0  then @adv_loans_child_id  when  @priority = 1  then @adv_loans_child_id when  @priority = 3  then @adv_loans_child_id_three when  @priority = 4  then @adv_loans_child_id_four else @adv_loans_child_id_two end, @os_priciapl_open_amount,@principal_paid_amount, 
case when @os_priciapl_open_amount=0 then 0 else  @remaing_amount_adj_festival end, @interest_accured_adj , @interest_open_amount, @interest_amount_recovered_adj, @principal_amount_recovered_one_adj, 
@completed_installment,  @installment_amount, @paid_date, @paid_date, @completed_installment, @installment_amount, @status, @transidnew, @fm, @fy, @loanslno,1);
