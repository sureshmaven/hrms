CREATE PROCEDURE [dbo].[sp_insert_two_sub_Loans_priority2] @emp_code int, @loan_code varchar(20) ,@date_disburse_two date,
@loan_amount_two int , @interest_rate_two float, @interest_accured_two float, @principal_amount_recovered_two int,
@interest_amount_recovered_two float, @total_amount_recovered_two float,@principal_start_date_two date ,
@principal_end_date_two date ,@interest_start_date_two date,@interest_end_date_two date ,@total_principal_installments_two int ,
@total_interest_installments_two int, @os_principal_amount_two int,@os_interest_amount_two float, @os_total_balace_amount float,
@loanslno int
AS
...
if not exists (select * from pr_emp_adv_loans_child where loan_sl_no = @loanslno and priority = 2)
begin
...
values(@idnew0,
@loan_id, 2 ,@date_disburse_two,@loan_amount_two,@interest_rate_two, @interest_accured_two, @principal_amount_recovered_two , 
@interest_amount_recovered_two ,@total_amount_recovered_two , 2 ,@principal_start_date_two, @principal_end_date_two,
@interest_start_date_two,@interest_end_date_two,@total_principal_installments_two,@total_interest_installments_two ,
@os_principal_amount_two, @os_interest_amount_two, 0 ,@os_total_balace_amount,
case when @os_principal_amount_two= 0 then 1 else 0 end,
case when @os_interest_amount_two= 0 then 1 else 0 end,
