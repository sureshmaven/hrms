...
--exec [sp_dm_adhoc] 175,'201906'

create procedure [dbo].[sp_dm_adhoc](@employeecode int,@pmonth varchar(20))
as 
begin
...
p.TGASSN as dd_telangana_officers_assn,p.Netpay as net_amount,p.Deductions as deductions_amount, 0 as active,1000 as trans_id,p.SBASICDA as spl_da,p.SBASICALW as spl_allw, 'Adhoc' as spl_type, 1 as final_process,p.CEOALLW as err_ceoallw 
into #pr_emp_payslip
from TEST2.dbo.PAdhslip p  join [TEST2].dbo.pempmast m on p.EID = m.EID 
where m.empid=@employeecode and year(p.PayDate)=left(@pmonth,4) and month(p.paydate)=right(@pmonth,2);
--p.paydate between '2019-04-01' and GETDATE() and m.empid=@employeecode and year(p.PayDate)=left(@pmonth,4) and month(p.paydate)=right(@pmonth,2);
...
--get codes
insert into #codes_master select id, name,code,type from pr_earn_field_master where code!='' and id not in(1,13) union all 
select id, name,code,type from pr_allowance_field_master where code!='' union all select id, name,code,type from dm_adhoc_earn 
--union all
--select id,name,code,type from pr_earn_field_master where name='CEO Allowance';
...
(select top 1 id from pr_emp_payslip where fm= @fm and fy=@fy and emp_code = @employeecode and spl_type='Adhoc'), @ID ,
...
insert into #codes_master_allded select id, name,code,type from pr_deduction_field_master where code!='';
...
(select top 1 id from pr_emp_payslip where fm= @fm and fy=@fy and emp_code = @employeecode and spl_type='Adhoc'), @IDsss ,
