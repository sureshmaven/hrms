...
--exec [sp_dm_encashment_present] 844,'202009' 
...
set @pmonthnew=SUBSTRING(@pmonth,0,5);
...
where  m.Empid = @employeecode and e.pmonth=@pmonth)
--p.paydate between '2019-04-01' and GETDATE() and e.pmonth>201903 and m.Empid = @employeecode and e.pmonth=@pmonth)
begin
...
(select TotalExperience from employees where EmpId=@employeecode),3,'Encashment',@pmonthnew,(select top 1 LeaveBalance from EmpLeaveBalance 
where empid=(select id from employees where empid=@employeecode) and LeaveTypeId=3 order by [year] desc),(select p.EncashDays from test2.dbo.PEncashment p join test2.dbo.PEMPMAST m on p.eid=m.eid where m.Empid=@employeecode and p.pmonth=@pmonth),(select Branch from employees where empid=@employeecode),
...
(select top 1 LeaveBalance from EmpLeaveBalance where empid=(select id from employees where empid=@employeecode) and LeaveTypeId=3 order by [year] desc),
1,0,@fy,@fm,null);
end
