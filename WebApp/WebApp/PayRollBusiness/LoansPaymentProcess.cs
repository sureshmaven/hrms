...
{
    class LoansPaymentProcess : BusinessBase
    {
        ...
        //Pf Loan 1
        private async Task<bool> PfLoan1Payments(DataTable dtLoan, DataTable dtFm, double instAmt, char PaymentType, DataTable dtPrevPartPay)
        {
            double intrestAmt = 0;
            int intrestAmt1 = 0;
            int NewNumIndex = 0;
            ...
            try
            {
                ...
                interest_paid_amount = intrestAmt;
                //added new
                if (instAmt != 0 && Install_amt != 0 && (instAmt == Install_amt || instAmt >= Install_amt))
                {
                ...
                sbqry.Append("INSERT INTO pr_emp_adv_loans_adjustments ([id],[emp_adv_loans_mid],[emp_adv_loans_child_mid],[principal_open_amount]," +
                   ...
                   "" + no_of_installments_cover + "," + Install_amt + "," + Completed_installments + ", @transidnew,'" + fm + "'," + fy + "," + interest_accured + "," + interest_paid_amount + "," + Loan_Sl_No + ");");
        ...
        //fest loan
        private async Task<bool> FestLoanPayments(DataTable dtLoan, DataTable dtFm, double instAmt, char PaymentType)
        {
            bool bRet = false;
            //Declarations
            int NewNumIndex = 0;
            ...
            try
            {
                ...
                sbqry.Append("INSERT INTO pr_emp_adv_loans_adjustments ([id],[emp_adv_loans_mid],[emp_adv_loans_child_mid],[principal_open_amount]," +
                   ...
                   "" + no_of_installments_cover + ",'" + installments_paid_date + "'," + instAmt + "," +
                   "" + no_of_installments_cover + "," + Install_amt + "," + Completed_installments + ", @transidnew,'" + fm + "'," + fy + "," + Loan_Sl_No + ");");
        ...
        public async Task<bool> RevertLoansData(int loan_id, int child_id, int adj_id, double instAmt, double intinstAmt, bool prin_recovered_flag, bool int_recovered_flag, double Prin_Paid_amt, double Int_Paid_amt, double Int_accured, int loan_type_mid, int Month, int Year, char PaymentType)
        {
            //sbqry.Append(GenNewTransactionString());
            sbqry.Clear();
            bool bRet = false;
            double dedamt_from_os_interest = Int_Paid_amt + Int_accured;
            double dedamt_from_total_amount_recovered = Prin_Paid_amt + Int_Paid_amt;

            if (loan_type_mid == 16 || loan_type_mid == 17 || loan_type_mid == 18 || loan_type_mid == 19 || loan_type_mid == 20 || loan_type_mid == 21)
            {
                dedamt_from_os_interest = 0;
            }
            try
            { 

                if (Prin_Paid_amt > 0 || Int_Paid_amt > 0)
                {
                    sbqry.Append("update pr_emp_adv_loans set completed_installment = completed_installment-1, " +
                 "remaining_installment = remaining_installment-1 where id=" + loan_id + " " +
                 "and loan_type_mid=" + loan_type_mid + ";");
                }

                if (Prin_Paid_amt > 0 && Int_Paid_amt == 0)
                {
                    sbqry.Append("update pr_emp_adv_loans_child set interest_accured = (interest_accured -" + Int_accured + "), " +
    "principal_amount_recovered =(principal_amount_recovered - " + Prin_Paid_amt + ")," +
    "interest_amount_recovered =(interest_amount_recovered - " + Int_Paid_amt + ")," +
    " total_amount_recovered =(total_amount_recovered - " + dedamt_from_total_amount_recovered + ")," +
    " os_principal_amount =(os_principal_amount + " + Prin_Paid_amt + "), " +
    "os_interest_amount =(os_interest_amount - " + dedamt_from_os_interest + ")  where id=" + child_id + " and emp_adv_loans_mid= " + loan_id + "");
                }
                else if (Int_Paid_amt > 0 && Prin_Paid_amt == 0)
                {
                    sbqry.Append("update pr_emp_adv_loans_child set interest_accured = (interest_accured -" + Int_accured + "), " +
"principal_amount_recovered =(principal_amount_recovered - " + Prin_Paid_amt + ")," +
"interest_amount_recovered =(interest_amount_recovered - " + Int_Paid_amt + ")," +
...
"os_interest_amount =(os_interest_amount + " + dedamt_from_os_interest + ")  where id=" + child_id + " and emp_adv_loans_mid= " + loan_id + "");
                ...
                {
                    //dedamt_from_os_interest = dedamt_from_os_interest - Int_accured;
                    sbqry.Append("update pr_emp_adv_loans_child set interest_accured = (interest_accured -" + Int_accured + "), " +
"principal_amount_recovered =(principal_amount_recovered - " + Prin_Paid_amt + ")," +
"interest_amount_recovered =(interest_amount_recovered - " + Int_Paid_amt + ")," +
" total_amount_recovered =(total_amount_recovered-" + dedamt_from_total_amount_recovered + ")," +
...
"os_interest_amount =(os_interest_amount - " + dedamt_from_os_interest + ")  where id=" + child_id + " and emp_adv_loans_mid= " + loan_id + "");
                ...
                sbqry.Append("delete from  pr_emp_adv_loans_adjustments where id = " + adj_id + "AND month(fm)=" + Month + " and year(fm)=" + Year + " and  payment_type!='Full Clearing' ");

                //sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.U, "pr_emp_adv_loans", loan_id.ToString(), loan_id.ToString()));
        ...
        public async Task<bool> InstallmentPartPayments(int loan_id, double instAmt, char PaymentType)
        {
            bool bRet = false;
            try
            {
                ...
                if (dtLoans.Rows.Count > 0)
                {
                    ...
                    {
                        bRet = await House2DLoanPayments(dtLoans, dtFm, instAmt, PaymentType, dtPartPay);
                    }
        ...
        //4w loan
        private async Task<bool> FourWheelerLoanPayments(DataTable dtLoan, DataTable dtFm, double instAmt, char PaymentType, DataTable dtPrevPartPay)
        {
            bool bRet = false;
            double Prin_bal_amt = 0;
            ...
            try
            {
                ...
                else if (Os_Principal_Amt == 0 && Os_Intrst_Amt > 0)
                {
                    ...
                    //added new
                    if (instAmt != 0 && Intrest_Install_amt != 0 && (instAmt == Intrest_Install_amt || instAmt >= Intrest_Install_amt))
                    {
                ...
                else
                {
                    ...
                    sbqry.Append("INSERT INTO pr_emp_adv_loans_adjustments ([id],[emp_adv_loans_mid],[emp_adv_loans_child_mid],[principal_open_amount]," +
                       ...
                       "" + Completed_installments + "," + Install_amt + "," + Completed_installments + ", @transidnew,'" + fm + "'," + fy + "," + Loan_Sl_No + ");");
                    sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.I, "pr_emp_adv_loans_adjustments", "@idnew" + NewNumIndex + "", ""));
                }
                Os_Principal_Amt = Prin_bal_amt;
                ...
                {
                    sbqry.Append("UPDATE pr_emp_adv_loans_child SET Interest_recovered_flag = 1,active=0 where emp_adv_loans_mid=" + LnPkid + " ;");
        ...
        private async Task<bool> TwoWheelerLoanPayments(DataTable dtLoan, DataTable dtFm, double instAmt, char PaymentType, DataTable dtPrevPartPay)
        {
            bool bRet = false;
            double Prin_Open_amt = 0;
            double Prin_bal_amt = 0;
            ...
            try
            {
                ...
                ThisMonthIntrestAmt = intrestAmt;
                //when loan adjustment
                if (Os_Principal_Amt < instAmt && Os_Principal_Amt > 0)
                {
                    Prin_Open_amt = Os_Principal_Amt;
                    ...
                    {
                        Interest_Open_amt = Os_Intrst_Amt;
                        Os_Intrst_Amt = (Os_Intrst_Amt - loan_adj_amt)+ intrestAmt;
                        ThisMonthIntrestAmt = intrestAmt;
                        intrestAmt = Os_Intrst_Amt;
                        Interest_Paid_amt = loan_adj_amt;
                        Interest_amount_recovered = Interest_amount_recovered + Interest_Paid_amt;
                ...
                else
                {
                    ...
                    sbqry.Append("INSERT INTO pr_emp_adv_loans_adjustments ([id],[emp_adv_loans_mid],[emp_adv_loans_child_mid],[principal_open_amount]," +
                        ...
                        "[payment_type],[payment_mode],[cash_paid_on],[covered_installments],[installments_paid_date],[amount_paid],[installments_paid]," +
                        "[installments_amount],[active],[trans_id],[fm],[fy],loan_sl_no) values "
                       + "(@idnew" + NewNumIndex + "," + LnPkid + "," + ChLnPkid + "," + Prin_Open_amt + "," + Prin_Paid_amt + "," + Prin_bal_amt + "," + ThisMonthIntrestAmt + "," +
                       "" + Interest_Open_amt + "," + Interest_Paid_amt + "," + Interest_Bal_amt + ",'" + payment_mode + "','" + payment_mode + "', '" + installments_paid_date + "'," +
                       "" + Completed_installments + ",'" + installments_paid_date + "'," + instAmt + "," +
                       "" + Completed_installments + "," + Install_amt + "," + Completed_installments + ", @transidnew,'" + fm + "'," + fy + "," + Loan_Sl_No + ");");
                    sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.I, "pr_emp_adv_loans_adjustments", "@idnew" + NewNumIndex + "", ""));
        ...
        //Housing Loand 2D
        private async Task<bool> House2DLoanPayments(DataTable dtLoan, DataTable dtFm, double instAmt, char PaymentType, DataTable dtPrevPartPay)
        {
            ...
            try
            {
                ...
                else if (Os_Principal_Amt >= 0 && Principal_recovered_flag == false && Os_Principal_Amt1 > 0)
                {
                  ...
                  "" + no_of_installments_cover1 + ",'" + installments_paid_date + "'," + instAmt + "," +
                  "" + no_of_installments_cover1 + "," + Install_amt + "," + Completed_installments + ", @transidnew+1,'" + fm + "'," + fy + "," + Loan_Sl_No + ");");
                    sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.I, "pr_emp_adv_loans_adjustments", "@idnew" + NewNumIndex + "", ""));
                }
                ...
                else if (Principal_recovered_flag == true && Principal_recovered_flag1 == true && Interest_recovered_flag == false && Interest_recovered_flag1 == false && loan_adj_amt != 0)
                {
                  ...
                  "" + no_of_installments_cover1 + "," + Amount_Paid + "," + Completed_installments + ", @transidnew+1,'" + fm + "'," + fy + "," + Loan_Sl_No + ");");
                    sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.I, "pr_emp_adv_loans_adjustments", "@idnew" + NewNumIndex + "", ""));
                }
                ...
                sbqry.Append(GetTransactionTouchStringArr(Transaction_Touch_Type.U, "pr_emp_adv_loans_child", ChLnPkid1.ToString(), instAmt.ToString()));
                //4. parent loan update
                string updInstallAmt = "";
                //update instalamt when Os_Principal_Amt Lessthan installmentAmt 
                ...
                if (Os_Principal_Amt == 0 && Principal_recovered_flag == false)
                {
                    sbqry.Append("UPDATE pr_emp_adv_loans_child SET principal_recovered_flag = 1 where id=" + ChLnPkid + " ;");
                    //Interest_installment_amt = Interest_Bal_amt / Interest_installments;
                }
                if (Os_Principal_Amt1 == 0 && Principal_recovered_flag1 == false && Principal_recovered_flag == true && Os_Principal_Amt == 0)
                {
                    sbqry.Append("UPDATE pr_emp_adv_loans_child SET principal_recovered_flag = 1 where id=" + ChLnPkid1 + " ;");
                ...
                {
                    sbqry.Append("UPDATE pr_emp_adv_loans_child SET Interest_recovered_flag = 1,active=0 where id=" + ChLnPkid + " ;");
                ...
                {
                    sbqry.Append("UPDATE pr_emp_adv_loans_child SET Interest_recovered_flag = 1,active=0 where id=" + ChLnPkid1 + " ;");
        ...
        private async Task<double> calcIntrestAmt(double osPrinc, double iRate, DateTime loanSactDate, DateTime dtFm, DateTime dtPartPay, DateTime dtPrevPartPay)
        {
            ...
            if ((dtFm.Year % 4 == 0 && dtFm.Year % 100 != 0) || (dtFm.Year % 400 == 0))
            {
                yrDays = 366;
            }
            else
