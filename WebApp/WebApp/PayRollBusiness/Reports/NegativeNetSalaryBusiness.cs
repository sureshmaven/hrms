...
{
    public class NegativeNetSalaryBusiness : BusinessBase
    {
        Mavensoft.DAL.Db.SqlHelper sh = new Mavensoft.DAL.Db.SqlHelper();

        public NegativeNetSalaryBusiness(LoginCredential loginCredential) : base(loginCredential)
        {
        ...
        public async Task<PayslipPdf> getPdfDetails(string EmpCode)
        {
            ...
            else
            {
                ...
                string qrygetid = "select id from pr_emp_payslip_netSalary where id=" + Convert.ToInt32(EmpCode) + ";";
                try
                {
                ...
                string qrySel = "SELECT format(g.fm,'MMMM yyyy') as fm,e.shortname,g.emp_code,b.Name,g.designation as Description,g.er_basic,g.er_da,g.er_cca," +
                                 ...
                                 "g.dd_income_tax,g.dd_prof_tax,g.dd_telangana_officers_assn,err_ceoallw, " +
                                 " format(e.DoJ,'dd-MM-yyyy') as DoJ ,format(e.RetirementDate,'dd-MM-yyyy') as RetirementDate,gen.pf_no " +
                                 "FROM pr_emp_payslip_netSalary g " +
                ...
                string qrySel2 = "select distinct g.all_name,g.all_amount " +
                                ...
                                " where payslip_mid=" + pid + " and all_amount>0 and all_name!='Employee TDS' ;";
                ...
                try
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (Convert.ToDecimal(drEmp["er_basic"]) > 0)
                        {
                            string basic = AddZerosAfterDecimal(drEmp["er_basic"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "Basic", Amount = basic });
                        }
                        if (Convert.ToDecimal(drEmp["er_da"]) > 0)
                        {
                            string da = AddZerosAfterDecimal(drEmp["er_da"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "DA", Amount = da });
                        }
                        if (Convert.ToDecimal(drEmp["er_cca"]) > 0)
                        {
                            string cca = AddZerosAfterDecimal(drEmp["er_cca"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "CCA", Amount = cca });
                        }

                        if (Convert.ToDecimal(drEmp["er_hra"]) > 0)
                        {
                            string hra = AddZerosAfterDecimal(drEmp["er_hra"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "HRA", Amount = hra });
                        }
                        if (Convert.ToDecimal(drEmp["er_interim_relief"]) > 0)
                        {
                            string interim = AddZerosAfterDecimal(drEmp["er_interim_relief"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "Interim Relief", Amount = interim });
                        }
                        if (Convert.ToDecimal(drEmp["spl_da"]) > 0)
                        {
                            string splda = AddZerosAfterDecimal(drEmp["spl_da"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "Spcl. DA", Amount = splda });
                        }
                        if (Convert.ToDecimal(drEmp["spl_allw"]) > 0)
                        {
                            string splall = AddZerosAfterDecimal(drEmp["spl_allw"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "Spl. Allow ", Amount = splall });
                        }
                        if (Convert.ToDecimal(drEmp["err_ceoallw"]) > 0)
                        {
                            string splall = AddZerosAfterDecimal(drEmp["err_ceoallw"].ToString());
                            allwList.Add(new PayslipPdfAlwDed { Perticular = "CEO Allowance ", Amount = splall });
                        }
                        if (Convert.ToDecimal(drEmp["dd_provident_fund"]) > 0)
                        {
                            string providentfund = AddZerosAfterDecimal(drEmp["dd_provident_fund"].ToString());
                            dedList.Add(new PayslipPdfAlwDed { Perticular = "Provident Fund", Amount = providentfund });
                        }
                        if (Convert.ToDecimal(drEmp["dd_income_tax"]) > 0)
                        {
                            string incometax = AddZerosAfterDecimal(drEmp["dd_income_tax"].ToString());
                            dedList.Add(new PayslipPdfAlwDed { Perticular = "Income tax", Amount = incometax });
                        }
                        if (Convert.ToDecimal(drEmp["dd_prof_tax"]) > 0)
                        {
                            string proftax = AddZerosAfterDecimal(drEmp["dd_prof_tax"].ToString());
                            dedList.Add(new PayslipPdfAlwDed { Perticular = "Professional Tax", Amount = proftax });

                        }
                        if (Convert.ToDecimal(drEmp["dd_club_subscription"]) > 0)
                        {
                            string clubsubscription = AddZerosAfterDecimal(drEmp["dd_club_subscription"].ToString());
                            dedList.Add(new PayslipPdfAlwDed { Perticular = "Club Subsriptions", Amount = clubsubscription });
                        }

                        if (Convert.ToDecimal(drEmp["dd_telangana_officers_assn"]) > 0)
                        {
                            string telanganofficers = AddZerosAfterDecimal(drEmp["dd_telangana_officers_assn"].ToString());
                            dedList.Add(new PayslipPdfAlwDed { Perticular = "Telangana Officers Association", Amount = telanganofficers });
                        }
                        if (Convert.ToDecimal(drEmp["er_telangana_inc"]) > 0)
                        {
                            string telanganainc = AddZerosAfterDecimal(drEmp["er_telangana_inc"].ToString());
                        ...
                        //{
                        paypdf.grossamount = drEmp["gross_amount"].ToString();
                        paypdf.deductionsamount = AddZerosAfterDecimal(drEmp["deductions_amount"].ToString());
                        paypdf.netamount = AddZerosAfterDecimal(drEmp["net_amount"].ToString());
                    ...
                    {
                        if (Convert.ToDecimal(drAllw["all_amount"]) > 0)
                        {
                            string allamount = AddZerosAfterDecimal(drAllw["all_amount"].ToString());
                    ...
                    {
                        if (Convert.ToDecimal(drDed["amount"]) > 0)
                        {
                            string allamount = AddZerosAfterDecimal(drDed["amount"].ToString());
                    ...
                    {
                        if (Convert.ToDecimal(drDed1["dd_amount"]) > 0)
                        {
                            string allamount = AddZerosAfterDecimal(drDed1["dd_amount"].ToString());
                    ...
                    {
                        if (Convert.ToDecimal(drDedloans["dd_amount"]) > 0)
                        {
                            string allamount = AddZerosAfterDecimal(drDedloans["dd_amount"].ToString());
        ...
        public async Task<PayslipPdf> SendPdfDetails(string EmpCode)
        {
            ...
            string sendmailling = "SELECT e.OfficalEmailId " +
                                  "FROM Employees e " +
                                  "JOIN pr_emp_payslip_netSalary g ON e.EmpId = g.emp_code" +
        ...
        public async Task<IList<Payslippdf>> GetNetSalaryReportdata(string emp_code, string pdftypes, string Month)
        {
            ...
            try
            {
                ...
                if ((str.Year >= CheckMonth.Year && str.Month >= CheckMonth.Month) || ((str.Year > CheckMonth.Year && str.Month < CheckMonth.Month)))
                {
                    ...
                    if (emp_code == "")
                    {
                        query = "SELECT ps.id as Id,e.shortname,ps.emp_code,ps.designation as Description,ps.gross_amount,ps.deductions_amount,ps.spl_type,ps.net_amount FROM pr_emp_payslip_netSalary ps " +
        ...
        //    try
        //    {
        //        query = "SELECT ps.id as Id,e.shortname,FORMAT(ps.fm, 'MMM yyyy') as emp_code,d.Code as Description,ps.gross_amount,ps.deductions_amount,ps.spl_type,ps.net_amount FROM pr_emp_payslip ps " +
        //      "JOIN Employees e ON e.EmpId = ps.emp_code " +
        //      "JOIN Branches b ON b.Id = e.Branch " +
        //      "JOIN Designations d ON d.Id = e.CurrentDesignation " +
