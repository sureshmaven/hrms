...
namespace BusLogic
{
    public class Promotion : CommonService
    {
        private string _dbConnStr;
        public Promotion(string dbConnStr) : base(dbConnStr)
        {
        ...
        public void Promotions()
        {
            try
            {
                string empid = getAllEmpIdspromotionTransfers();
                ...
                if (empids != "")
                {
                    ...
                    foreach (var emp in arrEmps)
                    {
                        ...
                        var desig = arremp[3];
                        updQry += "UPDATE Employees SET Department =" + dept + ",Branch=" + Branches + ",CurrentDesignation=" + desig + " WHERE Id=" + arremp[0] + "; ";
                    ...
                    string qryLog = "INSERT INTO hrms_scheduler_log([dttime],[Type],[Action],[Key1],[Val1]) "
                     + "VALUES(getdate(),'Promotion','Update','Emp Ids','" + empids + "' )";
        ...
        private string getAllEmpIdspromotionTransfers()
        {
            string retStr = "";

            string currDate = DateTime.Now.ToString("yyyy-MM-dd");

            string selQry = " Select   EmpId,NewDepartment,NewBranch,NewDesignation from Employee_Transfer where EffectiveFrom='" + currDate + "' AND [Type]='Promotion' order by UpdatedDate desc  ";
