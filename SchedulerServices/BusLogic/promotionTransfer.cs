...
namespace BusLogic
{
    public class PromotionTransfer : CommonService
    {
        private string _dbConnStr;
        public PromotionTransfer(string dbConnStr) : base(dbConnStr)
        {
            _dbConnStr = dbConnStr;
        }
        public void PromotionTransfers()
        {
            try
            {
                string empid = getAllEmpIdsFrompromTransfers();
                ...
                if (empids != "")
                {
                    ...
                    string qryLog = "INSERT INTO hrms_scheduler_log([dttime],[Type],[Action],[Key1],[Val1]) "
                     + "VALUES(getdate(),'PromotionTransfer','Update','Emp Ids','" + empids + "' )";
        ...
        private string getAllEmpIdsFrompromTransfers()
        {
            string retStr = "";

            string currDate = DateTime.Now.ToString("yyyy-MM-dd");

            string selQry = "Select  EmpId,NewDepartment,NewBranch,NewDesignation from Employee_Transfer where EffectiveFrom='" + currDate + "' AND [Type]='PromotionTransfer' order by UpdatedDate desc  ";
